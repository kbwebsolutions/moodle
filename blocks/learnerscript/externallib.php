...
class block_learnerscript_external extends external_api {
    ...
    public static function rolewiseusers($roleid, $term, $page, $_type, $reportid, $action, $maximumSelectionLength) {
        global $PAGE, $DB;
        ...
        if ((has_capability('block/learnerscript:managereports', $context) ||
            ...
            if ($roles == -1) {
                ...
                $user_list = (new schedule)->rolewiseusers($roles, $term, $page, $reportid);
            }
            $terms_data = array();
    ...
    public static function roleusers($reportid, $scheduleid, $selectedroleid, $roleid, $term, $type, $bullkselectedusers) {
        global $PAGE, $DB;
        ...
        if ((has_capability('block/learnerscript:managereports', $context) ||
            ...
            if ($roleid == -1) {
                ...
                $userslist = (new schedule)->schroleusers($reportid, $scheduleid, $type,
                                                    $roleid, $term, $bullkselectedusers);
            }
            $terms_data = array();
    ...
    public static function generate_plotgraph_parameters() {
        return new external_function_parameters(
            array(
                ...
                'categoryid' => new external_value(PARAM_INT, 'category id of category', false),
                'filteruserid' => new external_value(PARAM_INT, 'user id of filter', false),
    ...
    public static function generate_plotgraph($reportid, $courseid, $categoryid, $filteruserid, $cmid, $status, $userid,
        ...
        $reportclass->courseid = (isset($courseid) && $courseid != SITEID) ? $courseid : ($reportclass->params['filter_courses'] ? $reportclass->params['filter_courses'] : SITEID);
        $reportclass->categoryid = (isset($reportclass->params['filter_coursecategories']) && $reportclass->params['filter_coursecategories'] != 0) ? $reportclass->params['filter_coursecategories'] : ($reportclass->params['filter_coursecategories'] ? $reportclass->params['filter_coursecategories'] : 0);
        $reportclass->filteruserid = (isset($reportclass->params['filter_users']) && $reportclass->params['filter_users'] != 0) ? $reportclass->params['filter_users'] : ($reportclass->params['filter_users'] ? $reportclass->params['filter_users'] : 0);
        $reportclass->status = $status;

        $reportclass->currentcourseid = $reportclass->courseid > SITEID ? $reportclass->courseid : $reportclass->config->courseid;

        if (!$singleplot && $report->type != 'statistics') {
            $singleplot = $reporttype;
        } else if (!$singleplot && $report->type == 'statistics' && !$reportdashboard) {
            $singleplot = $reporttype;
        }
        ...
        if (!empty($components['plot']['elements'])) {
            ...
            foreach ($components['plot']['elements'] as $key => $e) {
                if ((($reportclass->reporttype && $reportclass->reporttype != 'table' && $e['id'] == $reportclass->reporttype) || ($reportclass->reporttype == '' && $i == 0)) && empty($e['formdata']->calcs)) {
                    if (isset($e['formdata']->sorting) && !empty($e['formdata']->sorting)) {
                        $reportclass->sqlorder = $e['formdata']->columnsort . ' ' . $e['formdata']->sorting;
                    }
                    $reportclass->length = $e['formdata']->limit;
                    break;
                }
            }
        }
        ...
        if ($singleplot == 'table') {
            ...
            if(empty($reportclass->finalreport->table->data)) {
                ...
                if (!empty($reportclass->finalreport->table->head)) {
                    ...
                    $reporttype = get_string('report_' . $report->type, 'block_learnerscript');
                    $return['reportname'] = (new ls)->get_reporttitle($reporttype, $basicparams);
    ...
    public static function importreports($total, $current, $errorreportspositiondata = array(), $lastreportposition = 0) {
        ...
        $pluginsettings = new block_learnerscript_licence_setting('block_learnerscript/lsreportconfigstatus',
