...
function block_learnerscript_plotforms_ajaxform($args) {
    ...
    require_once($CFG->dirroot . '/blocks/learnerscript/components/' . $args->component . '/' . $args->pname . '/plugin.class.php');
    $pluginclassname = 'plugin_' . $args->pname;
    ...
    require_once($CFG->dirroot . '/blocks/learnerscript/components/' . $args->component . '/' . $args->pname . '/form.php');
    $classname = $args->pname . '_form';
    $comp = $args->component;
    ...
    $mform = new $classname($formurl, compact('comp', 'cid', 'id', 'pluginclass', 'compclass',
                    'report', 'reportclass'), 'post', '', null, true, $ajaxformdata);
    ...
    if (!empty($ajaxformdata) && $mform->is_validated()) {
        ...
        if ($validated && $validateddata) {
            ...
            if ($args->cid) {
                ...
                $componentelements = $componentsdata[$component]['elements'];
...
function block_learnerscript_schreportform_ajaxform($args) {
    ...
    if (!empty($args->jsonformdata)) {
        parse_str($args->jsonformdata, $ajaxformdata);
        if (!empty($ajaxformdata)) {
            if ((!is_array($ajaxformdata['users_data']) || $ajaxformdata['users_data'] == '_qf__force_multiselect_submission')) {
    ...
    if ((has_capability('block/learnerscript:managereports', $context) ||
        ...
        list($schusers, $schusersids) = (new schedule)->userslist($reportid, $scheduleid, $ajaxformdata['schuserslist']);
        $exportoptions = (new ls)->cr_get_export_plugins();
        $frequencyselect = (new schedule)->get_options();
        if ($ajaxformdata['frequency']) {
        ...
        if (!empty($ajaxformdata) && $scheduleform->is_validated()) {
            ...
            if ($validateddata) {
                try {
                    ...
                    $fromform->roleid = $ajaxformdata['role'];
...
function block_learnerscript_sendreportemail_ajaxform($args) {
    ...
    if (!empty($args->jsonformdata)) {
        parse_str($args->jsonformdata, $ajaxformdata);
        if (!empty($ajaxformdata)) {
            if ((!is_array($ajaxformdata['email']) ||
