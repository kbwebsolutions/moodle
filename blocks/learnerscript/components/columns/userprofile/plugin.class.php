...
class plugin_userprofile extends pluginbase{
    ...
    public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0){
        ...
        $scormreportid = $DB->get_field('block_learnerscript', 'id', array('type' => 'myscorm'));
        ...
        switch ($data->column) {
            ...
            case 'scorms':
                ...
                $scormurl = new moodle_url('/blocks/learnerscript/viewreport.php',
                    array('id' => $scormreportid, 'filter_users' => $row->userid, 'filter_coursecategories' => $row->categoryid));
                if(empty($scormpermissions) || empty($scormreportid)){
                    $row->{$data->column} = $row->quizes;
                } else{
                    $row->{$data->column} = html_writer::tag('a', $row->scorms,
                    array('href' => $scormurl));
                }
            break;
            case 'badges':
            ...
            case 'completedcoursesgrade':
                $row->{$data->column} = (isset($row->completedcoursesgrade))? $row->completedcoursesgrade : '--';
                break;
            case 'progress':
                return "<div class='spark-report' id='spark-report$row->userid' data-sparkline='$row->progress; progressbar' data-labels = 'progress' >" . $row->progress . "</div>";
            break;
            case 'status':
