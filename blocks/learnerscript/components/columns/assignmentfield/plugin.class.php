...
class plugin_assignmentfield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        $categoryrecord = $DB->get_record('course', array('id' => $assignmentrecord->course));
        switch($data->column){
            case 'name':
                ...
                if(is_siteadmin()){
                    $url = new moodle_url('/mod/'.$module.'/view.php',
                             array('id' => $row->activityid, 'action' => 'grading'));
                }else {
                    $url = new moodle_url('/mod/'.$module.'/view.php',
            ...
            case 'course':
                ...
                if(empty($reportid) || empty($checkpermissions)){
                    ...
                    $assignmentrecord->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$reportid.'&filter_courses='.$assignmentrecord->course.'&filter_coursecategories='.$categoryrecord->category.'" />'.$coursename.'</a>';
                }
            break;
            case 'duedate':
