        ...
        switch ($data->column) {
            case 'coursefullname':
                ...
                if(empty($coursereportid) || empty($checkpermissions)){
	                ...
	                $row->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$coursereportid.'&filter_courses='.$row->courseid.'&filter_coursecategories='.$row->categoryid.'" />'.$row->coursefullname.'</a>';
	            }
            break;
        }
