...
class plugin_activityfield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        $courserecord = $DB->get_record('course_modules',array('id'=>$row->activityid));
            switch ($data->column) {
                case 'groupmode':
                ...
                case 'course':
                    ...
                    if(empty($reportid) || empty($checkpermissions)){
                        ...
                        $courserecord->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$reportid.'&filter_courses='.$courserecord->course.'&filter_coursecategories='.$row->categoryid.'" />'.$coursename.'</a>';
                    }
                break;
                case 'visible':
                ...
                case 'module':
                    ...
                    $courserecord->{$data->column} = $activityicon1 . ucfirst($moduletype);
                break;
                case 'section':
        ...
        return (isset($courserecord->{$data->column})) ? $courserecord->{$data->column} : $row->{$data->column};
    }
