...
class plugin_userfield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        if (strpos($data->column, 'profile_') === 0) {
            ...
            if ($profiledata = $DB->get_records_sql($sql, array($row->userid))) {
        ...
        $userprofilereport = $DB->get_field('block_learnerscript', 'id', array('type'=> 'userprofile'));
        $userrecord = $DB->get_record('user',array('id'=>$row->userid));
        ...
        if ($this->report->type == 'userprofile' || empty($userprofilereport) || empty($checkpermissions)) {
            $userrecord->fullname .= html_writer::tag('a', fullname($userrecord),
                        array('href' => $CFG->wwwroot.'/user/profile.php?id='.$row->userid.''));
        }else {
            $userrecord->fullname .= html_writer::tag('a', fullname($userrecord),
                                    array('href' => $CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$userprofilereport.'&filter_users='.$row->userid.'&filter_coursecategories='.$row->categoryid.''));
        }
        ...
        if($CFG->messaging){
            ...
            //                                        array('class' => 'icon sendsms','id'=>"sendsms_" . $this->reportinstance . "_" . $row->userid,
            //                                             'onclick'=>'(function(e){
            //                                                require("block_learnerscript/report").sendmessage({userid: '.$row->userid.', reportinstance: ' . $this->reportinstance . '}) })(event)')).
            //                     '</sup>';
            //added by Raghuvaran
            $userrecord->fullname .= "<sup id='communicate'>";
            $userrecord->fullname .= html_writer::start_span('ls icon sendsms', array('id'=>"sendsms_" . $this->reportinstance . "_" . $row->userid,
                                                         'onclick'=>'(function(e){
                                                            require("block_learnerscript/helper").sendmessage({userid: '.$row->userid.', reportinstance: ' . $this->reportinstance . '}, \''.$userfullname.'\'); e.stopImmediatePropagation(); }) (event)'));
