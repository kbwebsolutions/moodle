...
class plugin_quizzes extends pluginbase{
	...
	public function execute($data,$row,$user,$courseid,$starttime=0,$endtime=0){
        ...
        case 'notattemptedusers';
          $row->{$data->column} =  $row->notattemptedusers;
        break;
        case 'inprogressusers';
          $row->{$data->column} =  $row->inprogressusers;
        break;
        case 'completedusers';
          $row->{$data->column} =  $row->completedusers;
        break;
        case 'gradepass':
        ...
        break;
	case 'avggrade':
              $row->{$data->column} = isset($row->avggrade) ? $row->avggrade : '--' ;
            break;
		}
