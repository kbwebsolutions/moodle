    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        switch ($data->column) {
            case 'numviews':
                ...
                return html_writer::link("$CFG->wwwroot/blocks/learnerscript/viewreport.php?id=$reportid&filter_courses=$row->course&filter_activity=$row->activityid&filter_coursecategories=$row->categoryid", get_string('numviews', 'report_outline', $row), array("target" => "_blank"));
            break;
            case 'lowestgrade':
            case 'highestgrade':
            case 'avggrade':
                $row->{$data->column} = isset($row->{$data->column}) ? $row->{$data->column} : '--';
            break;
            case 'totaltimespent':
