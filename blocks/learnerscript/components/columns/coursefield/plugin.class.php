...
class plugin_coursefield extends pluginbase {
    ...
    public function execute($data, $row, $user, $courseid, $starttime = 0, $endtime = 0) {
        ...
        $coursereportid = $DB->get_field('block_learnerscript', 'id', array('type'=>'courseprofile'));
            ...
            switch ($data->column) {
                ...
                case 'fullname':
                    ...
                    if($this->report->type == 'courseprofile' || empty($coursereportid) || empty($checkpermissions)){
                        ...
                        $courserecord->{$data->column} = '<a href="'.$CFG->wwwroot.'/blocks/learnerscript/viewreport.php?id='.$coursereportid.'&filter_courses='.$courserecord->id.'&filter_coursecategories='.$courserecord->category.'" />'.$courserecord->{$data->column}.'</a>';
                    }
                break;
                case 'category':
