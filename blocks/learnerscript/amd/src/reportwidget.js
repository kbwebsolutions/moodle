define(['jquery',
    ...
    return reportwidget = {
        ...
        CreateDashboardTile: function(args) {
            ...
            if (typeof filters['filter_coursecategories'] == 'undefined') {
                var filter_coursecategories = $('#ls_categoryid').val();
                if (filter_coursecategories != 0) {
                    filters['filter_coursecategories'] = filter_coursecategories;
                }
            }
                            ...
                            $(data.plot.categorydata).each(function(k, v) {
                                if (data.plot.categorydata.length == 1) {
                                    if (!isNaN(tabledata[0][k])) {
                                        $("#inst" + args.blockinstanceid + " .tiles_information table tr").append('<td><h1> ' + tabledata[0][k] + ' </h1><p>' + data.plot.name + '</p></td>');
                                    } else {
                                        $("#inst" + args.blockinstanceid + " .tiles_information table tr").append('<td><p>' + data.plot.name + '  </p><h5><b>' + tabledata[0][k] + '</b></h5></td>');
                                    }
                                } else {
        ...
        CreateDashboardwidget: function(args) {
            ...
            if (typeof args.filters['filter_coursecategories'] == 'undefined') {
                var filter_coursecategories = $('#ls_categoryid').val();
                if (filter_coursecategories != 0) {
                    args.filters['filter_coursecategories'] = filter_coursecategories;
                }
            }
            ...
            if (typeof args.singleplot != 'undefined') {
