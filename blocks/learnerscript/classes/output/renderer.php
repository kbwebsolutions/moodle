...
class renderer extends plugin_renderer_base {
	...
	public function viewreport($report, $context, $reportclass) {
		...
		// 	$this->print_tabs($reportclass, 'viewreport');
		// }
		if ($report->type !== 'statistics') {
			$plots = (new ls)->get_components_data($report->id, 'plot');
			...
			if ($debug OR $CFG->debugdisplay OR $reportclass->config->debug) {
				$debugsql = true;
			}
		}

		// $filterbutton = $OUTPUT->single_button('javascript:void(0)', get_string('filters','block_learnerscript'));
		//  print_collapsible_region_start(' ', 'filters', $filterbutton , false, true);
		...
		$filterform = new \block_learnerscript\output\filtertoggleform($filterform);
		echo $this->render($filterform);

		//$filterform = $reportclass->print_filters(true);
		if($calcbutton){
			// echo '<button class="calculate_button" onclick="(function(e){ require(\'block_learnerscript/report\').reportCalculations({reportid:'.$report->id.'}) })(event)">
			// Calculations</button>';
		...
		$export = explode(',', $reportclass->config->export);
			// if (!empty($reportclass->basicparams) && $report->type !='statistics' && $report->type !='sql')  {
