...
class report_courseprofile extends reportbase implements report {
    ...
    public function __construct($report, $reportproperties) {
        ...
        $this->basicparams = [['name' => 'coursecategories'], ['name' => 'courses', 'singleselection' => false, 'placeholder' => false,
                                'maxlength' => 5]];
        $this->parent = false;
        $this->exports = false;
    }
    public function get_all_elements($sqlorder = '', $conditionfinalelements = array()) {
        ...
        $coursessql .= "SELECT c.id,c.fullname AS course, c.*, c.id AS courseid, c.category AS categoryid ";
        ...
        if (in_array('progress', $this->selectedcolumns)) {
            $coursessql .= ", ROUND(IF((COUNT(DISTINCT cc.userid)/COUNT(DISTINCT ue.userid) * 100) > 0, (COUNT(DISTINCT cc.userid)/COUNT(DISTINCT ue.userid) * 100), 0),2) AS progress";
        }
        ...
        if (in_array('numviews', $this->selectedcolumns)) {
            $coursessql .= ", (SELECT COUNT(DISTINCT lsl.userid)  
                        ...
                        (SELECT COUNT('X')  FROM {logstore_standard_log} lsl JOIN {user} u ON u.id = lsl.userid WHERE
                            lsl.crud = 'r' AND lsl.contextlevel = 50  AND lsl.anonymous = 0 AND lsl.userid > 2 AND
    ...
    public function create_report($blockinstanceid = null) {
        ...
        $courseids = isset($this->params['filter_courses']) &&
                        $this->params['filter_courses'] > 0 ? $this->params['filter_courses'] : $this->userid;
