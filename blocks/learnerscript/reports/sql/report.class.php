...
class report_sql extends reportbase {
    ...
    function get_all_elements($sql) {
        ...
        preg_match("/(GROUP BY|group by)[\r\t|\r\n|\r\s]+(.*)/", $sql, $groupmatch, PREG_OFFSET_CAPTURE, 0);
        preg_match("/^(SELECT|select)[\r\t|\r\n|\r\s]+(.*)/", $sql, $matches, PREG_OFFSET_CAPTURE, 0);
        ...
        $nolimitrecords = $remoteDB->get_recordset_sql($sql);
        $totalrecords = [];
        ...
        if (preg_match('/\b(INSERT|INTO|CREATE)\b/i', $sql)) {
            ...
            $results = $remoteDB->get_recordset_sql($sql, null, $this->start, $this->length);
        }
    ...
    function get_rows() {
        ...
        if (isset($config->querysql)) {
            ...
            if ($rs = $this->get_all_elements($sql)) {
