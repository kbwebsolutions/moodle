...
class report_noofviews extends reportbase implements report {
    ...
    public function get_all_elements() {
        ...
        if(!isset($this->params['filter_activity'])){
            ...
            $this->params['filter_activity'] = array_shift(array_keys($this->filterdata));
        }
        ...
        if (!empty($this->params['filter_activity'])) {
            $concatsql .= " AND lsl.contextinstanceid IN ($this->cmid) AND lsl.contextlevel = 70 ";
        }
        ...
        $selectsql  = "SELECT COUNT(lsl.id) as views, CONCAT(u.firstname, ' ' , u.lastname) as learner, lsl.userid as userid,  c.category AS categoryid";
        ...
        $sql        = " FROM {logstore_standard_log} lsl
                        ...
                        WHERE lsl.crud = 'r' AND lsl.userid > 2 AND u.confirmed = 1 AND u.deleted = 0 $concatsql 
                        $datefiltersql";
