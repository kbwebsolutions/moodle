...
$themeisccot = $theme === 'ccot';
$notajaxscript = !defined('AJAX_SCRIPT') || AJAX_SCRIPT == false;
// The code inside this conditional block is to be executed prior to page rendering when the theme is set to ccot and
...
if ($themeisccot && $notajaxscript) {
    ...
    if (empty($SESSION->theme_ccot_last_course) || $SESSION->theme_ccot_last_course != $COURSE->id) {
        $USER->editing = 0;
        $SESSION->theme_ccot_last_course = $COURSE->id;
    }

    if (isset($SESSION->wantsurl)) {
        // We are taking a backup of this because it can get unset later by core.
        $SESSION->ccotwantsurl = $SESSION->wantsurl;
    }
}
...
$THEME->prescsscallback = 'theme_ccot_get_pre_scss';
$THEME->scss = function($theme) {
    return theme_ccot_get_main_scss_content($theme);
};
$THEME->csspostprocess = 'theme_ccot_process_css';
...
if ($themeisccot && $notajaxscript) {
    if (empty($CFG->ccotpageinit) && !empty($PAGE)) {
        $CFG->ccotpageinit = true;
        $PAGE->initialise_theme_and_output();

        // Modify $PAGE to use ccot requirements manager.
        $ccotpman = new ccot_page_requirements_manager();
        $ccotpman->copy_page_requirements();
    }
