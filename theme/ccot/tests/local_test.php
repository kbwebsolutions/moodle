...
namespace theme_ccot\tests;

use theme_ccot\local;
use theme_ccot\ccot_base_test;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_ccot_local_test extends ccot_base_test {
     ...
     * ccot theme settings page. Creates a file, sets a theme
    ...
    protected function fake_site_image_setting_upload($filename) {
        ...
        $filepath = $CFG->dirroot.'/theme/ccot/tests/fixtures/'.$filename;

        $fs = \get_file_storage();

        $fs->delete_area_files($syscontext->id, 'theme_ccot', 'poster');

        $fs->create_file_from_pathname($filerecord, $filepath);
        \set_config('poster', $filename, 'theme_ccot');
     ...
     * ccot theme settings page. Deletes a file, sets a theme
    ...
    protected function fake_site_image_setting_cleared() {
        ...
        $fs->delete_area_files($syscontext->id, 'theme_ccot', 'coverimage');

        \set_config('poster', '', 'theme_ccot');
        local::process_coverimage($syscontext);
    }
    ...
    public function test_poster_image_upload() {
        ...
        foreach ($fixtures as $filename => $shouldberesized) {
            ...
            $this->assertContains('/theme_ccot/coverimage/', $css);
    ...
    protected function fake_course_image_setting_upload($filename, $context) {
        ...
        $fs->create_file_from_pathname($filerecord, $filepath);
        \set_config('coverimage', $filename, 'theme_ccot');
    }
    ...
    public function test_course_grade() {
        ...
        set_config('showcoursegradepersonalmenu', 1, 'theme_ccot');
