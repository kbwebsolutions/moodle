...
@theme @theme_ccot
Feature: When the moodle theme is set to ccot, sets a color per category.
  ...
  Background:
    ...
    And the following config values are set as admin:
      | category_color | {"5":"#00FF00","30":"#FF0000"} | theme_ccot |
    And the following config values are set as admin:
  ...
  Scenario: Load all classes in each category hierarchy.
    ...
    And I check body for classes "theme-ccot"
    And I follow "Miscellaneous"
    And I check body for classes "theme-ccot,category-1"
    And I follow "Courses"
    And I follow "Browse all courses"
    And I follow "Cat 5"
    And I check body for classes "theme-ccot,category-5"
    And I follow "Cat 10"
    And I check body for classes "theme-ccot,category-5,category-10"
    And I follow "Cat 20"
    And I check body for classes "theme-ccot,category-5,category-10,category-20"
  ...
  Scenario: Check category colors from nearest parent in hierarchy.
    Given the following config values are set as admin:
      | category_color | {"5":"#00FF00","10":"#FF0000"} | theme_ccot |
