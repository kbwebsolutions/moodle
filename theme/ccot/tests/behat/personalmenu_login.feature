...
@theme @theme_ccot
Feature: When the moodle theme is set to ccot,
  ...
  Scenario: User opens and closes login menu using call-to-action button on site homepage
    Given I click on "#page-mast .js-ccot-pm-trigger" "css_element"
    Then ".ccot-pm-login-form" "css_element" should be visible
    And I follow "Close"
    Then ".ccot-pm-login-form" "css_element" should not be visible

  @javascript
  Scenario: User logs in and does not see the personal menu, if option turned off
    Given the following config values are set as admin:
      | personalmenulogintoggle | 0 | theme_ccot |
    Given I follow "Log in"
    And I set the field "username" to "teacher1"
    And I set the field "password" to "teacher1"
    And I press "Log in"
    Then "#ccot-pm" "css_element" should not be visible
  ...
  Scenario: User logs in and sees the personal menu, then closes it and re-opens without changing section
    ...
    And I set the field "password" to "teacher1"
    And I press "Log in"
    Then "#ccot-pm" "css_element" should be visible
    ...
    Then "#ccot-pm" "css_element" should be visible
    And I follow "Close"
    And "#ccot-pm" "css_element" should not be visible
    And "#section-1" "css_element" should be visible
