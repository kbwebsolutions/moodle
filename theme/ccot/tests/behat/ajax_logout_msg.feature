...
@theme @theme_ccot
Feature: When the moodle theme is set to ccot, ajax failures due to log outs / expired sessions are reported correctly
  as session issues.
  ...
  Scenario: Teacher get's login status warning when trying to manage sections if logged out.
    ...
    And I log out via a separate window
    When I click on "#section-2 .ccot-visibility.ccot-hide" "css_element"
    Then I should see "You are logged out"
    ...
    Then "#section-2" "css_element" should exist
    And I log out via a separate window
    And I click on "#section-2 .ccot-highlight" "css_element"
    Then I should see "You are logged out"
    ...
    Then I should see "Moving \"Untitled Topic\"" in the "#ccot-footer-alert" "css_element"
  ...
  Scenario: Teacher get's login status warning when trying to manage assets if logged out.
    ...
    And I log out via a separate window
    When I click on ".ccot-activity[data-type='Assignment'] a.js_ccot_hide" "css_element"
    Then I should see "You are logged out"
    ...
    And I click on ".ccot-activity[data-type='Assignment'] a.ccot-edit-asset-more" "css_element"
    And I log out via a separate window
    When I click on ".ccot-activity[data-type='Assignment'] a.js_ccot_duplicate" "css_element"
    Then I should see "You are logged out"
    ...
    And I click on ".ccot-activity.modtype_assign .ccot-asset-move img[title='Move \"Test assignment\"']" "css_element"
    Then I should see "Moving \"Test assignment\""
    And I log out via a separate window
    When I click on "li#section-1 li.ccot-drop.asset-drop div.asset-wrapper" "css_element"
    Then I should see "You are logged out"
  ...
  Scenario: User account is created with force password change on login enabled, personal menu shows a warning when
    ...
    And I open the personal menu
    Then I should see "You must change your password before using the personal menu." in the "#ccot-pm-content .force-pwd-warning" "css_element"
