...
use \theme_ccot\activity,
    \theme_ccot\ccot_base_test;

global $CFG;
require_once($CFG->dirroot . '/mod/assign/tests/base_test.php');

/**
 * Testing for theme/ccot/classes/activity.php
 *
 * @package  theme_ccot
 * @copyright  2017 Blackboard Inc.
 * @license  http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_ccot_acitvity_test extends ccot_base_test {
    ...
    public function test_get_calendar_activity_events() {
        ...
        $ccotevent = reset($events);
        $this->assertEquals($due, $ccotevent->timestart);
        $this->assertEquals($due, $ccotevent->timesort);

        // Test extension set.
        $this->extend_assign_deadline($assign->id, $student->id, $extension);
        // Test that the ccot calendar includes extensions to assignments.
        $events = activity::get_calendar_activity_events($tstart, $tend, $courses);
        $ccotevent = reset($events);
        $this->assertEquals($extension, $ccotevent->timestart);
        $this->assertEquals($extension, $ccotevent->timesort);
        ...
        // Test that the ccot calendar event is the same as the core event
        // (with the exception of the timestart + timesort fields).
        unset($ccotevent->timestart);
        unset($ccotevent->timesort);
        unset($coreevent->timestart);
        unset($coreevent->timesort);
        $this->assertEquals($ccotevent, $coreevent);
    }
    ...
    public function test_user_activity_events() {
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovdgroupdue, $ccotevent->timestart);
        $this->assertEquals($ovdgroupdue, $ccotevent->timesort);
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovduserdue, $ccotevent->timestart);
        $this->assertEquals($ovduserdue, $ccotevent->timesort);
    ...
    public function test_instance_activity_dates_quiz_assign() {
        ...
        $this->setUser($student);
        $quizdates = \theme_ccot\activity::instance_activity_dates($course->id, $cm, '', '');
        ...
        $dates1 = \theme_ccot\activity::instance_activity_dates($course->id, $cm, '', '');
        $dates2 = \theme_ccot\activity::instance_activity_dates($course->id, $cm2, '', '');
        ...
        $this->setUser($student);
        $assigndates = \theme_ccot\activity::instance_activity_dates($course->id, $cm, '', '');
