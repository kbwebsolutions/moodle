...
/**
 * Main ccot initialising function.
 */
define(['jquery', 'core/log', 'theme_ccot/headroom', 'theme_ccot/util', 'theme_ccot/personal_menu',
        'theme_ccot/cover_image', 'theme_ccot/progressbar', 'core/templates', 'core/str'],
    function($, log, Headroom, util, personalMenu, coverImage, ProgressBar, templates, str) {

        'use strict';

        /* eslint-disable camelcase */
        M.theme_ccot = M.theme_ccot || {};
        ...
        var movePHPErrorsToHeader = function() {
            ...
            if (msgs.length) {
                ...
                var errorcont = $('<div id="footer-error-cont"><h3>' +
                    M.util.get_string('debugerrors', 'theme_ccot') +
                    '</h3><hr></div>');
                ...
                var errorlink = $('<a class="footer-error-link btn btn-danger" href="#footer-error-cont">' +
                    M.util.get_string('problemsfound', 'theme_ccot') + ' <span class="badge">' + (msgs.length) + '</span></a>');
        ...
        var bodyClasses = function() {
            var extraclasses = [];
            if ($('#notice.ccot-continue-cancel').length) {
            ...
            $(window).on('popstate hashchange', function(e) {
                ...
                if (newHash !== lastHash) {
                    if (location.hash === '#primary-nav') {
                        ...
                        $('#page, #moodle-footer, #js-ccot-pm-trigger, #logo, .skiplinks').css('display', '');
                        if (onCoursePage()) {
        ...
        var recentUpdatesFix = function() {
            $('#ccot-course-footer-recent-activity .info').each(function() {
                $(this).appendTo($(this).prev());
            });
            $('#ccot-course-footer-recent-activity .head .name').each(function() {
        ...
        return {
             ...
             */
            ccotInit: function(courseConfig, pageHasCourseContent, siteMaxBytes, forcePassChange,
                               messageBadgeCountEnabled, userId, sitePolicyAcceptReqd, inAlternativeRole) {

                // Set up.
                M.cfg.context = courseConfig.contextid;
                M.ccotTheme = {forcePassChange: forcePassChange};
                ...
                $(document).ready(function() {
                    ...
                    // Allow deeplinking to bs tabs on ccot settings page.
                    if ($('#page-admin-setting-themesettingccot').length) {
                    ...
                    if ((onModSettings || onCourseSettings || onSectionSettings) && pageNotInBlacklist) {
                        ...
                        $('#mform1 > fieldset').not(vital).wrapAll('<div class="ccot-form-advanced col-md-4" />');

                        // Add expand all to advanced column.
                        $(".ccot-form-advanced").append($(".collapsible-actions"));
                        // Add collapsed to all fieldsets in advanced, except on course edit page.
                        if (!$('#page-course-edit').length) {
                            $(".ccot-form-advanced fieldset").addClass('collapsed');
                        }
                        ...
                        $(mainForm).wrap('<div class="ccot-form-required col-md-8" />');
                        ...
                        $("#page-mod-resource-mod .ccot-form-advanced #id_modstandardelshdr .fcontainer").append(description);
                        $("#page-mod-resource-mod .ccot-form-advanced #id_modstandardelshdr .fcontainer").append(showdescription);
                        ...
                        $("#page-mod-assign-mod .ccot-form-advanced #id_modstandardelshdr .fcontainer").append(filemanager);
                        $("#page-mod-assign-mod .ccot-form-required .fcontainer").append(duedate);

                        // Move availablity at the top of advanced settings.
                        var availablity = $('#id_visible').closest('.form-group').addClass('ccot-form-visibility');
                        ...
                        $(label).text(M.util.get_string('visibility', 'theme_ccot') + ' ');
                        ...
                        if ($("#page-course-edit").length) {
                            ...
                            (function(){
                                return str.get_strings([
                                    {key: 'showallsectionsdisabled', component: 'theme_ccot'},
                                    {key: 'disabled', component: 'theme_ccot'}
                                ]);
                            })()
                                .then(function(strings) {
                                    var strMessage = strings[0];
                                    strDisabled = strings[1];
                                    return templates.render('theme_ccot/form_alert', {
                                        type: 'warning',
                                        classes: '',
                                        message: strMessage
                                    });
                        ...
                        $('.ccot-form-advanced').prepend(availablity);
                        ...
                        if ($("#page-mod-page-mod").length) {
                            ...
                            (function() {
                                return str.get_strings([
                                    {key: 'showappearancedisabled', component: 'theme_ccot'}
                                ]);
                            })()
                    ...
                    if (messageBadgeCountEnabled) {
                        require(
                            [
                                'theme_ccot/conversation_badge_count-lazy'
                    ...
                    var focustarget = $('#ccot-coverimagecontrol label');
                    if (focustarget && focustarget.length) {
                        focustarget.keypress(function (e) {
                            if (e.which === 13) {
                                $('#ccot-coverfiles').trigger('click');
                            }
                        });
                    }
                    ...
                    if (!$('.block_settings').length) {
                        ...
                        if (inAlternativeRole) {
                            ...
                            require(
                                [
                                    'theme_ccot/alternative_role_handler-lazy'
                    ...
                    var ccotHome = $('#ccot-home');
                    if (notificationsBtn.length && searchButton.length && adminMenuTrigger.length && ccotHome.length) {
                        // Update tab events because all elements have tabindex="0" and they are rendered funny.
                        require(
                            [
                                'theme_ccot/rearrange_tab_handler-lazy'
                            ], function(searchTabHandler) {
                                searchTabHandler.init([notificationsBtn, searchButton, adminMenuTrigger, ccotHome]);
                            }
                        );
                    }
