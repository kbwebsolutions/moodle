...
define(['jquery', 'core/log', 'core/ajax', 'core/str', 'core/templates', 'core/notification',
    'theme_ccot/util', 'theme_ccot/ajax_notification', 'theme_ccot/footer_alert'],
    ...
    return {
        init: function(courseLib) {
            ...
            var stopMoving = function() {
                $('body').removeClass('ccot-move-inprogress');
                $('body').removeClass('ccot-move-section');
                $('body').removeClass('ccot-move-asset');
                footerAlert.hideAndReset();
                $('.section-moving').removeClass('section-moving');
                $('.asset-moving').removeClass('asset-moving');
                $('.js-ccot-asset-move').removeAttr('checked');
                movingObjects = [];
            };
            ...
            var moveFailed = function() {
                ...
                footerAlert.setTitle(M.util.get_string('movefailed', 'theme_ccot', actname));
            ...
            var updateMovingMessage = function() {
                ...
                if (movingObjects.length === 1) {
                    var assetname = $(movingObjects[0]).find('.ccot-asset-link .instancename').html();
                    assetname = assetname || M.util.get_string('pluginname', 'label', assetname);
                    title = M.util.get_string('moving', 'theme_ccot', assetname);
                } else {
                    title = M.util.get_string('movingcount', 'theme_ccot', movingObjects.length);
                }
            ...
            var updateSectionNavigation = function(selector) {
                ...
                $.each(sections, function(idx, el) {
                    ...
                    templates.render('theme_ccot/course_section_navigation', navigation)
            ...
            var sectionDelete = function(e, el) {
                ...
                var doDelete = function() {
                    ...
                    var delProgress = M.util.get_string('deletingsection', 'theme_ccot', sectionName);
                    ...
                    var ajaxPromises = ajax.call([
                        {
                            methodname: 'theme_ccot_course_sections',
                            args: params
                        }
                    ], true, true);
                    ...
                    ajaxPromises[0]
                        .done(function(response) {
                            // Update TOC.
                            templates.render('theme_ccot/course_toc', response.toc)
                                .done(function(result) {
                                    $('#course-toc').html($(result).html());
                                    $(document).trigger('ccotTOCReplaced');
                ...
                var ok = M.util.get_string('deletesectionconfirm', 'theme_ccot');
            ...
            var assetAction = function(e, triggerEl) {
                ...
                var assetEl = $($(triggerEl).parents('.ccot-asset')[0]),
                ...
                var actionAJAX = function() {
                    if (!ajaxTracker.start(jsPendingKey, assetEl, '.ccot-edit-asset-more')) {
                        // Request already made.
                        return;
                    }
                ...
                var getErrorStrings = function() {
                    ...
                    return str.get_strings([
                        {key: errActionKey, component: 'theme_ccot'},
                        {key: errMessageKey, component: 'theme_ccot'}
                    ]);
                };
                ...
                getErrorStrings().then(function(strings) {
                    ...
                    if (action === 'delete') {
                        ...
                        var delTitle = M.util.get_string('confirm', 'moodle');
                        var ok = M.util.get_string('deleteassetconfirm', 'theme_ccot', plugindata.type);
                        var cancel = M.util.get_string('cancel', 'moodle');
            ...
            var ajaxReqMoveAsset = function(target) {
                ...
                if (target && !$(target).hasClass('ccot-drop')) {
            ...
            var ajaxReqMoveSection = function(dropzone) {
                ...
                ajaxReqMoveGeneral(params, function() {
                    ...
                    ajax.call([
                        {
                            methodname: 'theme_ccot_course_toc_chapters',
                            args: {
                                courseshortname: courseLib.courseConfig.shortname
                            },
                            done: function(response) {
                                // Update TOC.
                                templates.render('theme_ccot/course_toc_chapters', response.chapters)
                                    .done(function(result) {
                                        // Update chapters.
            ...
            var assetEditListeners = function() {
                var actionSelectors = '.ccot-asset-actions .js_ccot_hide, ';
                actionSelectors += '.ccot-asset-actions .js_ccot_show, ';
                actionSelectors += '.ccot-asset-actions .js_ccot_delete, ';
                actionSelectors += '.ccot-asset-actions .js_ccot_duplicate';
            ...
            var sectionActionListener = function(action, onComplete) {
                ...
                $('#region-main').on('click', '.ccot-section-editing.actions .ccot-' + action, function(e) {
                    ...
                    if (action === 'visibility') {
                        toggle = $(this).hasClass('ccot-hide') ? 0 : 1;
                    } else {
                    ...
                    var sectionActionsSelector = '#section-' + sectionNumber + ' .ccot-section-editing';
                    var actionSelector = sectionActionsSelector + ' .ccot-' + action;
                    ...
                    .fail(function(response) {
                        ...
                        if (action === 'visibility') {
                            errMessage = M.util.get_string('error:failedtochangesectionvisibility', 'theme_ccot');
                            errAction = M.util.get_string('action:changesectionvisibility', 'theme_ccot');
                        } else {
                            errMessage = M.util.get_string('error:failedtohighlightsection', 'theme_ccot');
                            errAction = M.util.get_string('action:highlightsectionvisibility', 'theme_ccot');
                        }
                        ajaxNotify.ifErrorShowBestMsg(response, errAction, errMessage).done(function() {
                            // Allow another request now this has finished.
                        ...
                        return templates.render('theme_ccot/course_action_section', response.actionmodel)
                        .then(function(result) {
                            $(actionSelector).replaceWith(result);
                            $(actionSelector).focus();
                            // Update TOC.
                            return templates.render('theme_ccot/course_toc', response.toc);
                        }).then(function(result) {
                            $('#course-toc').html($(result).html());
                            $(document).trigger('ccotTOCReplaced');
            ...
            var highlightSectionListener = function() {
                sectionActionListener('highlight', function(sectionNumber) {
                    ...
                    $notCurrent.each(function() {
                        var highlighter = $(this).find('.ccot-highlight');
            ...
            var deleteSectionListener = function() {
                $(document).on('click', '.ccot-section-editing.actions .ccot-delete', function(e) {
            ...
            var moveSectionListener = function() {
                ...
                $("#region-main").on('click', '.ccot-section-editing.actions .ccot-move', function(e) {
                    e.stopPropagation();
                    e.preventDefault();

                    $('body').addClass('ccot-move-inprogress');
                    footerAlertShowMove();
                    ...
                    $('body').addClass('ccot-move-section');

                    var title = M.util.get_string('moving', 'theme_ccot', sectionName);
                    footerAlert.setTitle(title);

                    $('.section-drop').each(function() {
                        var sectionDropMsg = M.util.get_string('movingdropsectionhelp', 'theme_ccot',
                            {moving: sectionName, before: $(this).data('title')}
                        );
                        $(this).html(sectionDropMsg);
                    });

                    footerAlert.setSrNotice(M.util.get_string('movingstartedhelp', 'theme_ccot', sectionName));
                });
            };
            ...
            var addAfterDrops = function() {
                if (document.body.id === "page-site-index") {
                    $('#region-main .sitetopic ul.section').append(
                        '<li class="ccot-drop asset-drop">' +
                        '<div class="asset-wrapper">' +
                        '<a href="#">' +
                        M.util.get_string('movehere', 'theme_ccot') +
                        '</a>' +
                        '</div>' +
                        '</li>');
                } else {
            ...
            var assetMoveListener = function() {
                $("#region-main").on('change', '.js-ccot-asset-move', function(e) {
                    e.stopPropagation();

                    var asset = $(this).parents('.ccot-asset')[0];

                    // Make sure after drop is at the end of section.
                    var section = $(asset).parents('ul.section')[0];
                    var afterdrop = $(section).find('li.ccot-drop.asset-drop');
                    $(section).append(afterdrop);

                    if (movingObjects.length === 0) {
                        // Moving asset - activity or resource.
                        // Initiate move.
                        var assetname = $(asset).find('.ccot-asset-link .instancename').html();

                        log.debug('Moving this asset', assetname);

                        var classes = $(asset).attr('class'),
                            regex = /(?=ccot-mime)([a-z0-9\-]*)/;
                        ...
                        $(asset).find('.js-ccot-asset-move').prop('checked', 'checked');

                        $('body').addClass('ccot-move-inprogress');
                        $('body').addClass('ccot-move-asset');
                    }
            ...
            var movePlaceListener = function() {
                $(document).on('click', '.ccot-move-note, .ccot-drop', function(e) {
                    log.debug('ccot drop clicked', e);
                    if (movingObjects) {
                        e.stopPropagation();
                        e.preventDefault();
                        if ($('body').hasClass('ccot-move-section')) {
                            ajaxReqMoveSection(this);
                        } else {
                            var target;
                            if ($(this).hasClass('ccot-drop')) {
                                target = this;
                            } else {
                                target = $(this).closest('.ccot-asset');
                            }
                            ajaxReqMoveAsset(target);
                        }
                    }
            ...
            var addListeners = function() {
                ...
                assetEditListeners();
                addAfterDrops();
                $('body').addClass('ccot-course-listening');
            };
