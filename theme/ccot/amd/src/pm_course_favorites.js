 ...
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/log', 'theme_ccot/model_view', 'theme_ccot/ajax_notification'],
    function($, ajax, notification, log, mview, ajaxNotify) {
        return function() {
            ...
            var reloadCourseCardTemplate = function(renderable, cardEl) {
                var dfd = $.Deferred();
                mview(cardEl, 'theme_ccot/course_cards');
            ...
            var moveOutOfFavorites = function(cardEl, onMoveComplete) {
                ...
                // When this is 0 we only have hidden courses, so container is #ccot-pm-courses-current-cards.
                var publishedcount = $('#ccot-pm-courses-current .coursecard:not([data-hidden="true"])').length;
                // Special stuff for when moving a hidden course.
                if ($(cardEl).data('hidden') === true && publishedcount > 0) {
                    container = '#ccot-pm-courses-hidden-cards';
                    // Open hidden courses section.
                    $('#ccot-pm-courses-hidden').addClass('state-visible');
                    $('#ccot-pm-courses-hidden-cards').collapse('show');
                } else {
                    window.console.log('not a hidden card');
                    container = '#ccot-pm-courses-current-cards';
                }
            ...
            var favoriteCourse = function(button) {
                ...
                var doAjax = function(jsid) {
                    return ajax.call([
                        {
                            methodname: 'theme_ccot_course_card',
                ...
                if (favorited === 1) {
                    ...
                    // Move to favorites.
                    moveCard(cardEl, '#ccot-pm-courses-current-cards .coursecard.favorited', '#ccot-pm-courses-current-cards', true,
                        function() {
                            doAjax(jsid);
                        }
                    );
             ...
             */
            $("#ccot-pm").on("click", ".favoritetoggle", function(e) {
                e.preventDefault();
                e.stopPropagation();
                favoriteCourse(this);
            });
