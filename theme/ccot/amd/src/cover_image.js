...
define(['jquery', 'core/log', 'core/ajax', 'core/notification', 'theme_ccot/ajax_notification'],
    function($, log, ajax, notification, ajaxNotify) {
        ...
        var addCoverImageAlert = function(id, msg) {
            ...
            if (!$(id).length) {
                $('#ccot-coverimagecontrol').before(
                    '<div id="' + id + '" class="ccot-alert-cover-image alert alert-warning" role="alert">' +
                    msg +
         ...
         */
        var state1 = function() {
            $('#ccot-changecoverimageconfirmation .ok').removeClass('ajaxing');
            $('#ccot-alert-cover-image-size').remove();
            $('#ccot-alert-cover-image-bytes').remove();
            $('label[for="ccot-coverfiles"]').removeClass('ajaxing');
            $('#ccot-changecoverimageconfirmation').removeClass('state-visible');
            $('label[for="ccot-coverfiles"]').addClass('state-visible');
            $('#ccot-coverfiles').val('');
        };
         ...
         */
        var state2 = function() {
            $('#ccot-alert-cover-image-upload-failed').remove();
            $('#ccot-changecoverimageconfirmation').removeClass('disabled');
            $('label[for="ccot-coverfiles"]').removeClass('state-visible');
            $('#ccot-changecoverimageconfirmation').addClass('state-visible');
        ...
        var coverImage = function(siteMaxBytes, ajaxParams) {
            ...
            $('#ccot-coverfiles').on('change', function(e) {
                ...
                $('label[for="ccot-coverfiles"]').addClass('ajaxing');
                ...
                reader.onload = (function(theFile) {
                    return function(e) {
                        ...
                        if (theFile.size > maxbytes) {
                            ...
                            var message = M.util.get_string('error:coverimageexceedsmaxbytes', 'theme_ccot', maxbytesstr);
                            addCoverImageAlert('ccot-alert-cover-image-bytes', message);
                            return;
                        } else {
                            $('#ccot-alert-cover-image-bytes').remove();
                        }
                        ...
                        $(img).on('load', function() {
                            if (img.width < 1024) {
                                addCoverImageAlert('ccot-alert-cover-image-size',
                                    M.util.get_string('error:coverimageresolutionlow', 'theme_ccot')
                                );
                            } else {
                                $('#ccot-alert-cover-image-size').remove();
                            }
            ...
            $('#ccot-changecoverimageconfirmation .ok').click(function() {
                ...
                $('#ccot-changecoverimageconfirmation .ok').addClass('ajaxing');
                $('#ccot-changecoverimageconfirmation').addClass('disabled');
                ...
                ajax.call([
                    {
                        methodname: 'theme_ccot_cover_image',
                        args: {params: ajaxParams},
                        done: function(response) {
                            state1();
                            if (!response.success && response.warning) {
                                addCoverImageAlert('ccot-alert-cover-image-upload-failed', response.warning);
                            }
                        },
            ...
            $('#ccot-changecoverimageconfirmation .cancel').click(function() {
            ...
            $('#ccot-coverimagecontrol').addClass('ccot-js-enabled');
        };
