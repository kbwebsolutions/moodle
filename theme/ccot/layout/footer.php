...
if ($this->page->user_is_editing() && $PAGE->pagetype == 'site-index') {
    $url = new moodle_url('/admin/settings.php', ['section' => 'themesettingccot'], 'admin-footnote');
    $link = html_writer::link($url, get_string('editcustomfooter', 'theme_ccot'), ['class' => 'btn btn-primary btn-sm']);
...
if (!empty($custommenu) && $this->page->user_is_editing() && $PAGE->pagetype == 'site-index') {
    ...
    $link = html_writer::link($url, get_string('editcustommenu', 'theme_ccot'), ['class' => 'btn btn-primary btn-sm']);
    $custommenu .= '<p class="text-right">'.$link.'</p>';
}


/* ccot main footer. */
echo '<div id="ccot-site-footer">';
if (!empty($footnote)) {
    echo '<div id="ccot-footer-content">';
    echo $footnote;
    echo '</div>';
}
/* Social media links. */
...
if (!empty($socialmedialinks)) {
    echo '<div id="ccot-socialmedia-links">' .$socialmedialinks. '</div>';
}
echo '</div>';
?>
