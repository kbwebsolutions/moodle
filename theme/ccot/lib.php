...
/**
 * Standard library functions for ccot theme.
 *
 * @package   theme_ccot
 ...
 */
function theme_ccot_process_site_coverimage() {
    $context = \context_system::instance();
    \theme_ccot\local::process_coverimage($context);
 ...
 * @return string
 */
function theme_ccot_process_css($css, theme_config $theme) {

    $css = theme_ccot_set_category_colors($css, $theme);

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_ccot_set_logo($css, $logo);
    ...
    $css = theme_ccot_set_customcss($css, $customcss);
 ...
 */
function theme_ccot_set_category_colors($css, $theme) {
    global $DB;
    ...
    $dbcategorycolors = get_config("theme_ccot", "category_color");
    ...
    if (!empty($categorycolors)) {
        ...
        $themedirectory = realpath(core_component::get_component_directory('theme_ccot'));
        ...
        foreach ($categories as $category) {
            ...
            $scss = str_replace('.theme-ccot {', $categoryselector, $brandscss);
 ...
 */
function theme_ccot_set_logo($css, $logo) {
    $tag = '/**setting:logo**/';
    if (is_null($logo)) {
        $replacement = '';
    } else {
        $replacement = "#ccot-home.logo, .ccot-logo-sitename {background-image: url($logo);}";
    }
    $css = str_replace($tag, $replacement, $css);
    return $css;
}
 ...
 */
function theme_ccot_set_customcss($css, $customcss) {
 ...
 * @return bool
 */
function theme_ccot_send_file($context, $filearea, $args, $forcedownload, $options) {
    ...
    $fullpath = "/$contextid/theme_ccot/$filearea/0/$filename";
    $fs = get_file_storage();
 ...
 * @return bool
 */
function theme_ccot_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    ...
    if ($context->contextlevel == CONTEXT_SYSTEM && in_array($filearea, $sysfileareas)) {
        $theme = theme_config::load('ccot');
        return $theme->setting_file_serve($filearea, $args, $forcedownload, $options);
    } else if (in_array($context->contextlevel, $coverimagecontexts)
        && $filearea == 'coverimage' || $filearea == 'coursecard') {
        theme_ccot_send_file($context, $filearea, $args, $forcedownload, $options);
    } else {
        send_file_not_found();
    }
}

function theme_ccot_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    global $PAGE;

    if ($PAGE->theme->name === 'ccot') {
        if ($iscurrentuser) {
...
function theme_ccot_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_ccot', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_ccot and not theme_boost (see the line above).
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/ccot/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/ccot/scss/post.scss');
 ...
 * @return array
 */
function theme_ccot_get_pre_scss($theme) {
    global $CFG;
