...
namespace theme_ccot\renderables;
...
class course_toc implements \renderable, \templatable{
    ...
    use \theme_ccot\output\general_section_trait;
    use \theme_ccot\renderables\trait_exportable;
        ...
        foreach ($modinfo->get_section_info_all() as $section => $thissection) {
            ...
            if ($canviewhidden) { // Teachers.
                if ($conditional) {
                    ...
                    $chapter->availabilitystatus = get_string('conditional', 'theme_ccot');
                    $chapter->classes .= 'conditional ';
                }
                if (!$thissection->visible) {
                    $chapter->availabilityclass = 'text-warning';
                    $chapter->availabilitystatus = get_string('notpublished', 'theme_ccot');
            ...
            if ($chapter->title == get_string('general')) {
                $chapter->title = get_string('introduction', 'theme_ccot');
            }

            if ($this->format->is_section_current($section)) {
                $chapter->iscurrent = true;
                $chapter->classes .= 'ccot-visible-section current ';
            }
