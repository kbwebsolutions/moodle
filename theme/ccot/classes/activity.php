...
class activity {
    ...
    public static function module_meta(\cm_info $mod) {
        ...
        if (method_exists('theme_ccot\\activity', $methodname)) {
            $meta = call_user_func('theme_ccot\\activity::' . $methodname, $mod);
        } else {
    ...
    protected static function std_meta(\cm_info $mod,
        ...
        $meta->submittedstr = get_string($submitstrkey, 'theme_ccot');
        $meta->notsubmittedstr = get_string('not'.$submitstrkey, 'theme_ccot');
        if (get_string_manager()->string_exists($mod->modname.'draft', 'theme_ccot')) {
            $meta->draftstr = get_string($mod->modname.'draft', 'theme_ccot');
        } else {
            $meta->drafstr = get_string('draft', 'theme_ccot');
        }

        if (get_string_manager()->string_exists($mod->modname.'reopened', 'theme_ccot')) {
            $meta->reopenedstr = get_string($mod->modname.'reopened', 'theme_ccot');
        } else {
            $meta->reopenedstr = get_string('reopened', 'theme_ccot');
        }
        ...
        if (has_capability('mod/assign:grade', $mod->context)) {
            ...
            if (method_exists('theme_ccot\\activity', $methodnsubmissions)) {
                $meta->numsubmissions = call_user_func('theme_ccot\\activity::'.$methodnsubmissions, $courseid, $mod->instance);
            }
            if (method_exists('theme_ccot\\activity', $methodnungraded)) {
                $meta->numrequiregrading = call_user_func('theme_ccot\\activity::'.$methodnungraded, $courseid, $mod->instance);
            }
        } else {
...
-- Exclude those people who can grade quizzes
    ...
    public static function get_events(
        ...
        \theme_ccot\calendar\event\container::ovd_init();
        $vault = \theme_ccot\calendar\event\container::get_event_vault();
    ...
    public static function user_activity_events($userorid, array $courses, $tstart, $tend, $cacheprefix = '',
        ...
        if (self::$phpunitallowcaching || !(defined('PHPUNIT_TEST') && PHPUNIT_TEST)) {
            $muc = \cache::make('theme_ccot', 'activity_deadlines');
