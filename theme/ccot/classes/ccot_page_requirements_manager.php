...
/**
 * ccot page requirements manager.
...
class ccot_page_requirements_manager extends \page_requirements_manager {
    /**
     ...
     * the ccot_page_requirements_manager.
     */
    public function copy_page_requirements() {
        ...
        foreach ($props as $prop) {
            ...
            $ccotmanprop = new \ReflectionProperty($this, $pname);
            // If the property is private or protected  set accessible, after the copy reset to not accessible.
            $isprotected = $ccotmanprop->isPrivate() || $ccotmanprop->isProtected();
            if ($isprotected) {
                $ccotmanprop->setAccessible(true);
            }
            $ccotmanprop->setValue($this, $pval);
            $ccotmanprop->setAccessible(!$isprotected);
        }
