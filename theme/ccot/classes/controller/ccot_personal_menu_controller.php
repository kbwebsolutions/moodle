...
namespace theme_ccot\controller;
 ...
 */
class ccot_personal_menu_controller extends controller_abstract {
    /**
    ...
    public function get_deadlines_action() {
        ...
        $deadlines = \theme_ccot\activity::upcoming_deadlines($USER->id);
        return json_encode([
    ...
    public function get_forumposts_action() {
        return json_encode(array(
            'html' => \theme_ccot\local::render_recent_forum_activity()
        ));
    }
    ...
    public function get_courseinfo_action() {
        ...
        $courseinfo = \theme_ccot\local::courseinfo($courseids);
        return json_encode(array(
            'info' => $courseinfo
        ));
    }
    ...
    public function get_loginstatus_action() {
        ...
        if (!$loggedin) {
            if (!empty($failedactionmsg)) {
                $return['loggedoutmsg'] = get_string('loggedoutfailmsg', 'theme_ccot', $failedactionmsg);
            } else {
                $return['loggedoutmsg'] = get_string('loggedoutmsg', 'theme_ccot');
            }
            $return['loggedouttitle'] = get_string('loggedoutmsgtitle', 'theme_ccot');
