...
use moodle_url;
use stdClass;
use theme_ccot\renderables\course_action_section_move;
use theme_ccot\renderables\course_action_section_visibility;
use theme_ccot\renderables\course_action_section_delete;
use theme_ccot\renderables\course_action_section_highlight;
use theme_ccot\renderables\course_section_navigation;
...
trait format_section_trait {
    ...
    protected function section_header($section, $course, $onsectionpage, $sectionreturn=null) {
        ...
        // ajax.
        $output = $PAGE->get_renderer('theme_ccot', 'core', RENDERER_TARGET_GENERAL);
        ...
        if ($sectiontitle == get_string('general') && $section->section == 0) {
            $classes = '';
            $sectiontitle = get_string('introduction', 'theme_ccot');
        }
        ...
        if ($sectiontitle == $testemptytitle && has_capability('moodle/course:update', $context)) {
            ...
            $o .= "<h2 class='sectionname'><a href='$url' title='".s(get_string('editcoursetopic', 'theme_ccot'))."'>";
            $o .= get_string('defaulttopictitle', 'theme_ccot')."</a></h2>";
        } else {
        ...
        if ($caneditcourse && $section->section != 0) {
            $o .= "<a class='ccot-drop section-drop' data-title='".
        ...
        if (has_capability('moodle/course:update', $context)) {
            if (!empty($sectiontoolsarray)) {
                ...
                $o .= html_writer::tag('div', $sectiontools, array(
                    'class' => 'js-only ccot-section-editing actions',
                    'role' => 'region',
                    'aria-label' => get_string('topicactions', 'theme_ccot')
                ));
            }
        }
        // Draft message.
        $drafticon = '<img aria-hidden="true" role="presentation" class="svg-icon" src="'.$output->image_url('/i/show').'" />';
        $o .= '<div class="ccot-draft-tag ccot-draft-section">'.$drafticon.' '.get_string('draft', 'theme_ccot').'</div>';

        // Current section message.
        $currenticon = '<img aria-hidden="true" role="presentation" class="svg-icon" src="'.$output->image_url('/i/marked').'" />';
        $o .= '<span class="ccot-current-tag">'.$currenticon.' '.get_string('current', 'theme_ccot').'</span>';
        ...
        if ($formattedinfo !== '') {
            $o .= '<div class="ccot-conditional-tag">'.$conditionalicon.' '.$formattedinfo.'</div>';
        }
        ...
        if (empty($summarytext) && $canupdatecourse) {
            $summarytext = "<p tabindex='0'>".get_string('defaultsummary', 'theme_ccot')."</p>";
            if ($section->section == 0) {
                $editorname = format_string(fullname($USER));
                $summarytext = "<p tabindex='0'>".get_string('defaultintrosummary', 'theme_ccot', $editorname)."</p>";
            }
        } else {
        ...
        if ($canupdatecourse) {
            ...
            $o .= '<a href="'.$url.'" class="edit-summary">'.$icon.get_string('editcoursetopic', 'theme_ccot'). '</a>';
        }
        $o .= "</div>";
    ...
    public function render_course_section_navigation(course_section_navigation $navigation) {
        return $this->render_from_template('theme_ccot/course_section_navigation', $navigation);
    }
    ...
    private function change_num_sections($course) {
        ...
        $url = new moodle_url('/theme/ccot/index.php', array(
            'sesskey'  => sesskey(),
            'action' => 'addsection',
        ...
        $required = '';
        $defaulttitle = get_string('title', 'theme_ccot');
        ...
        $heading = get_string('addanewsection', 'theme_ccot');
        $output = "<section id='ccot-add-new-section' class='clearfix' tabindex='-1'>
        <h3>$heading</h3>";
        ...
        $output .= "<label for='newsection' class='sr-only'>".get_string('title', 'theme_ccot')."</label>";
        if ($course->format === 'topics') {
            $output .= '<input class="h3" id="newsection" type="text" maxlength="250" name="newsection" '.$required;
            $output .= ' placeholder="'.s(get_string('title', 'theme_ccot')).'">';
        } else {
            $output .= '<h3>'.$defaulttitle.': '.$datesection.'</h3>';
        }
        $output .= '</div>';
        $output .= '<div class="form-group">';
        $output .= '<label for="summary">'.get_string('contents', 'theme_ccot').'</label>';
        ...
        $output .= html_writer::empty_tag('input', array(
            ...
            'name' => 'addtopic',
            'value' => get_string('createsection', 'theme_ccot'),
        ));
    ...
    public function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        $mcdataattributes = 'data-section="'.$section.'" data-toggle="modal" data-target="#ccot-modchooser-modal"';
        $modchooser = '
        <div class="col-sm-6 ccot-modchooser">
            <a href="#" class="'.$mcclass.'" '.$mcdataattributes.'>'.$icon.get_string('addresourceoractivity', 'theme_ccot').'</a>
        </div>';

        // Add zone for quick uploading of files.
        $upload = '<div class="col-sm-6">
            <form class="ccot-dropzone js-only">
                <label tabindex="0" for="ccot-drop-file-'.$section.'" class="ccot-dropzone-label">';
        $upload .= get_string('dropzonelabel', 'theme_ccot').'</label>
                <input type="file" multiple name="ccot-drop-file-'.$section.'" id="ccot-drop-file-'.$section;
        $upload .= '" class="js-ccot-drop-file sr-only"/>
            </form>
            </div>';
    ...
    /**
     * Always output the html for multiple sections, single section mode is not supported in ccot.
     *
     * @param stdClass $course The course entry from DB
    ...
    public function render_course_action_section_move(course_action_section_move $action) {
        ...
        return $this->render_from_template('theme_ccot/course_action_section', $data);
    }
