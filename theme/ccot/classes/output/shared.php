...
class shared extends \renderer_base {
    ...
    protected static function dndupload_add_to_course($course, $modnames) {
        ...
        $script = <<<EOF
            <script>
                var themeccotCourseFileHandlers = $json;
            </script>
EOF;
        ...
        $PAGE->requires->js_call_amd('theme_ccot/dndupload-lazy', 'init', $vars);
    }
    ...
    public static function page_requires_js() {
        ...
        $PAGE->requires->js_call_amd('theme_ccot/ccot', 'ccotInit', $initvars);
    ...
    public static function appendices() {
        ...
        foreach ($instances as $instance) { // Need to check enrolment methods for self enrol.
            if ($instance->enrol === 'self') {
                ...
                if (is_enrolled($coursecontext)) {
                    ...
                    if ($enrolurl) {
                        $selfenrol = true;
                        $enrolstr = get_string('unenrolme', 'theme_ccot');
                        break;
                    }
                } else {
        ...
        if (has_capability('moodle/course:update', $coursecontext)) {
            ...
            $settingsicon = '<img src="'.$iconurl.'" class="ccot-cover-icon svg-icon" alt="" role="presentation">';

            $links[] = array(
                'link' => 'course/edit.php?id='.$COURSE->id,
                'title' => $settingsicon.get_string('editcoursesettings', 'theme_ccot'),
            );
        }
        ...
        if (course_can_view_participants($coursecontext)) {
            ...
            $participanticons = '<div class="ccot-participant-icons">'.$participanticons.'</div>';
            $links[] = array(
        ...
        if (array_key_exists('pld', $localplugins) && has_capability('local/pld:editcourserules', $coursecontext)) {
            ...
            $pldname = get_string('pld', 'theme_ccot');
            $links[] = array(
        ...
        $coursetools = get_string('coursetools', 'theme_ccot');
    ...
    public static function print_student_dashboard() {
        ...
        $userpicture->class = 'userpicture ccot-icon'; // Icon class for margin.
        $userpicture->size = 100;
        $userpic = $OUTPUT->render($userpicture);

        $userboard  = '<div id="ccot-student-dashboard" class="row clearfix">';
        ...
        if ($COURSE->enablecompletion) {
            ...
            $userboard .= '<div class="col-xs-3 text-center ccot-student-dashboard-progress">';
            $userboard .= '<h4 class="h6">' .get_string('progress', 'theme_ccot'). '</h6>';
            $userboard .= '<div class="js-progressbar-circle ccot-progress-circle" value="' .round($progress->progress). '"></div>';
            $userboard .= '</div>';
        }
        ...
        if (has_capability('gradereport/overview:view', $coursecontext)) {
            ...
            $userboard .= '<div class="col-xs-3 text-center ccot-student-dashboard-grade">';
            $userboard .= '<h4 class="h6">' . get_string('grade') . '</h6>';
            $userboard .= '<a href="' . $moodleurl . '">';
            $userboard .= '<div class="js-progressbar-circle ccot-progress-circle ccot-progressbar-link" value="';
            $userboard .= s($coursegrade) . '"></div>';
            $userboard .= '</a>';
            $userboard .= '</div>';
        }

        $userboard .= '</div><!- close .ccot-user-dashboard ->';
        $userboard .= '<br>';
