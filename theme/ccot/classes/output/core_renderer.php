...
use navigation_node;
use user_picture;
use theme_ccot\local;
use theme_ccot\activity;
use theme_ccot\services\course;
use theme_ccot\renderables\settings_link;
use theme_ccot\renderables\bb_dashboard_link;
use theme_ccot\renderables\course_card;
use theme_ccot\renderables\course_toc;
use theme_ccot\renderables\featured_courses;

// We have to force include this class as it's on login and the auto loader may not have been updated via a cache dump.
require_once($CFG->dirroot.'/theme/ccot/classes/renderables/login_alternative_methods.php');
use theme_ccot\renderables\login_alternative_methods;
...
class core_renderer extends \theme_boost\output\core_renderer {
        ...
        if ($helpidentifier) {
            ...
            if ($helpidentifier === 'modulename') {
                ...
                if (!empty($mod) && isset($mod[$component]) && is_object($mod[$component]) && $mod[$component]->help) {
                    ...
                    $collapsablehelp = $this->render_from_template('theme_ccot/heading_help_collapse', $data);
                    $classnames .= ' d-inline';
                }
    ...
    public function course_toc() {
        ...
        return $this->render_from_template('theme_ccot/course_toc', $coursetoc);
    }
    ...
    public function get_course_image() {
        ...
        return \theme_ccot\local::course_coverimage_url($COURSE->id);
    }
    ...
    public function column_header_icon_link($langstring, $iconname, $url) {
        global $OUTPUT;
        $text = get_string($langstring, 'theme_ccot');
        $iconurl = $OUTPUT->image_url($iconname, 'theme');
        $icon = '<img class="svg-icon" role="presentation" src="' .$iconurl. '">';
        $link = '<a class="ccot-personal-menu-more" href="' .$url. '"><small>' .$text. '</small>' .$icon. '</a>';
        return $link;
    }
    ...
    public function mobile_menu_link($langstring, $iconname, $url) {
        global $OUTPUT;
        $alt = get_string($langstring, 'theme_ccot');
        $iconurl = $OUTPUT->image_url($iconname, 'theme');
    ...
    public function render_settings_link(settings_link $settingslink) {
        ...
        $gearicon = '<svg xmlns="http://www.w3.org/2000/svg" id="ccot-admin-icon" viewBox="0 0 100 100">
                        <title>'.get_string('admin', 'theme_ccot').'</title>
        ...
        $attributes = array(
            ...
            'title' => get_string('admin', 'theme_ccot'),
            'aria-label' => get_string('admin', 'theme_ccot'),
        );
    ...
    protected function render_badge_count() {
        ...
        if (!empty($CFG->messaging) && !empty(get_config('theme_ccot', 'messagestoggle'))) {
    ...
    protected function render_messages() {
        ...
        $messagesheading = get_string('messages', 'theme_ccot');
        $o = '<h2>'.$messagesheading.'</h2>';
        $o .= '<div id="ccot-personal-menu-messages"></div>';
    ...
    protected function render_forumposts() {
        ...
        $forumpostsheading = get_string('forumposts', 'theme_ccot');
        $o = '<h2>'.$forumpostsheading.'</h2>
        <div id="ccot-personal-menu-forumposts"></div>';
     ...
     * @return string
     */
    public function ccot_media_object($url, $image, $title, $meta, $content, $extraclasses = '', $attributes = '') {
        ...
        if (is_array($meta)) {
            $metastr = '<span class="ccot-media-meta">';
            foreach ($meta as $metaitem) {
                $metastr .= $metaitem.'<br>';
            }
            $metastr .= '</span>';
        } else if ($meta) {
            $metastr = '<span class="ccot-media-meta">' .$meta.'</span>';
        }
        ...
        return $this->render_from_template('theme_ccot/media_object', $data);
    }
    ...
    /**
     * Output moodle blocks and ccot wrapper with edit button.
     * @return string
     */
    public function ccot_blocks() {
        ...
        if ($oncoursepage && has_capability('moodle/course:update', $coursecontext)) {
            ...
            if ($PAGE->user_is_editing()) {
                ...
                $url->param('edit', 'on');
                $editstring = get_string('editcoursecontent', 'theme_ccot');
            }
    ...
    protected function render_callstoaction() {
        ...
        $mobilemenu = '<div id="ccot-pm-mobilemenu">';
        $mobilemenu .= $this->mobile_menu_link('courses', 'courses', '#ccot-pm-courses');
        $deadlines = $this->render_deadlines();
        if (!empty($deadlines)) {
            $columns[] = $deadlines;
            $mobilemenu .= $this->mobile_menu_link('deadlines', 'calendar', '#ccot-personal-menu-deadlines');
        }

        $graded = $this->render_graded();
        $grading = $this->render_grading();
        if (empty($grading)) {
            $gradebookmenulink = $this->mobile_menu_link('recentfeedback', 'grading', '#ccot-personal-menu-graded');
        } else {
            $gradebookmenulink = $this->mobile_menu_link('grading', 'grading', '#ccot-personal-menu-grading');
        }
        if (!empty($grading)) {
            $columns[] = $grading;
        ...
        if (!empty($messages)) {
            $columns[] = $messages;
            $mobilemenu .= $this->mobile_menu_link('messages', 'messages', '#ccot-personal-menu-messages');
        }

        $forumposts = $this->render_forumposts();
        if (!empty($forumposts)) {
            $columns[] = $forumposts;
            $mobilemenu .= $this->mobile_menu_link('forumposts', 'forumposts', '#ccot-personal-menu-forumposts');
        }
    ...
    protected function render_grading() {
        ...
        $gradingheading = get_string('grading', 'theme_ccot');
        $o = "<h2>$gradingheading</h2>";
        $o .= '<div id="ccot-personal-menu-grading"></div>';
    ...
    protected function render_graded() {
        ...
        $recentfeedback = get_string('recentfeedback', 'theme_ccot');
        $o = "<h2>$recentfeedback</h2>";
        $o .= '<div id="ccot-personal-menu-graded"></div>';
    ...
    protected function render_deadlines() {
        ...
        $deadlinesheading = get_string('deadlines', 'theme_ccot');
        $o = "<h2>$deadlinesheading</h2>";
        $o .= '<div id="ccot-personal-menu-deadlines"></div>';
    ...
    public function login_button() {
        ...
        $loginatts = [
            'aria-haspopup' => 'true',
            'class' => 'btn btn-primary ccot-login-button js-ccot-pm-trigger',
        ];
        if (!empty($CFG->alternateloginurl) or !empty($CFG->theme_ccot_disablequicklogin)) {
            $loginurl = $CFG->wwwroot.'/login/index.php';
            $loginatts = [
                'class' => 'btn btn-primary ccot-login-button',
            ];
        }
    ...
    public function render_course_card(course_card $card) {
        return $this->render_from_template('theme_ccot/course_cards', $card);
    }
    ...
    public function render_login_alternative_methods(login_alternative_methods $methods) {
        ...
        return $this->render_from_template('theme_ccot/login_alternative_methods', $methods);
    }
    ...
    public function personal_menu() {
        ...
        if (!isloggedin() || isguestuser()) {
            ...
            if ($PAGE->pagetype !== 'login-index') {
                return $this->render_from_template('theme_ccot/login', $data);
            } else {
                return '';
            }
        }
        ...
        $fullnamelink = '<a href="' .s($CFG->wwwroot). '/user/profile.php"
                    title="' .s(get_string('viewyourprofile', 'theme_ccot')). '"
        ...
        if (\core\session\manager::is_loggedinas()) {
            ...
            $realfullnamelink = '<br>' .get_string('via', 'theme_ccot'). ' ' .format_string(fullname($realuser, true));
        }
        ...
        $dashboardlink = [
            ...
            'title' => get_string('mybadges', 'theme_ccot')
        ];
        $gradelink = [
        ...
        $quicklinks = [$profilelink, $preferenceslink, $dashboardlink, $logoutlink];
        ...
        return $this->render_from_template('theme_ccot/personal_menu', $data);
    }
    ...
    public function personal_menu_trigger() {
        ...
        if (!isloggedin() || isguestuser()) {
            ...
            $menu = '<span class="hidden-xs-down">' .get_string('menu', 'theme_ccot'). '</span>';
            $badge = $this->render_badge_count();
            $linkcontent = $picture.$menu.$badge;
            $attributes = array(
                'aria-haspopup' => 'true',
                'class' => 'js-ccot-pm-trigger ccot-my-courses-menu',
                'id' => 'ccot-pm-trigger',
                'aria-controls' => 'ccot-pm',
            );
    ...
    public function cover_image_selector() {
        ...
        if (has_capability('moodle/course:changesummary', $PAGE->context)) {
            ...
            return $this->render_from_template('theme_ccot/cover_image_selector', $vars);
        }
        return null;
    }
    ...
    public function cover_carousel() {
        ...
        $data['slides'] = $slides;
        return $this->render_from_template('theme_ccot/carousel', $data);
    }
    ...
    public function page_heading($tag = 'h1') {
        ...
        if ($this->page->pagelayout == 'frontpage') {
            $heading .= '<p class="ccot-site-description">' . format_string($this->page->theme->settings->subtitle) . '</p>';
        }
        if ($this->page->user_is_editing() && $this->page->pagelayout == 'frontpage') {
            $url = new moodle_url('/admin/settings.php', ['section' => 'themesettingccot']);
            $link = html_writer::link($url,
                            get_string('changefullname', 'theme_ccot'),
    ...
    public function site_frontpage_news() {
        ...
        foreach ($discussions as $discussion) {
            ...
            $imgarr = \theme_ccot\local::extract_first_image($message);
            if ($imgarr) {
            ...
            $readmorebtn = "<a class='btn btn-secondary toggle' href='".
                ...
                get_string('readmore', 'theme_ccot')."</a>";
            ...
            if (!$imagestyle) {
                ...
                $newsimage = '<div class="news-article-image toggle"'.$imagestyle.' title="'.
                    get_string('readmore', 'theme_ccot').'"></div>';
            }
...
<div class="news-article clearfix">
    ...
    <div class="news-article-message" tabindex="-1">
        {$message}
        <div><hr><a class="ccot-action-icon ccot-icon-close toggle" href="#">
...
HTML;
        ...
        $actionlinks = html_writer::link(
            ...
            get_string('morenews', 'theme_ccot'),
    ...
    /**
     * add in additional classes that are used for ccot
    ...
    public function body_css_classes(array $additionalclasses = array()) {
        ...
        if (!empty($PAGE->theme->settings->resourcedisplay)) {
            $classes[] = 'ccot-resource-'.$PAGE->theme->settings->resourcedisplay;
        } else {
            $classes[] = 'ccot-resource-card';
        }

        // Add theme-ccot class so modules can customise css for ccot.
        $classes[] = 'theme-ccot';
    ...
    public function confirm($message, $continue, $cancel) {
        ...
        $output = $this->box_start('generalbox ccot-continue-cancel', 'notice');
    ...
    public function recent_forum_activity(Array $activities) {
        ...
        foreach ($activities as $activity) {
            ...
            $output .= $this->ccot_media_object($url, $picture, $fullname, $meta, $formattedsubject);
        }
        return $output;
    }
    ...
    public function deadlines(stdClass $eventsobj) {
        ...
        if (empty($events)) {
            return '<p class="small"'.$datafromcache.'>' . get_string('nodeadlines', 'theme_ccot') . '</p>';
        }
        ...
        foreach ($events as $event) {
            if (!empty($event->modulename)) {
                ...
                if (!has_capability('mod/assign:grade', $cm->context)) {
                    /** @var \theme_ccot_core_course_renderer $courserenderer */
                    $courserenderer = $PAGE->get_renderer('core', 'course', RENDERER_TARGET_GENERAL);
                    $activitymeta = activity::module_meta($cm);
                    $meta .= '<div class="ccot-completion-meta">' .
                        $courserenderer->submission_cta($cm, $activitymeta) .
                        '</div>';
                }
                $o .= $this->ccot_media_object($cm->url, $modimage, $eventtitle, $meta, '', '', $datafromcache);
            }
        }
        if (empty($o)) {
            return '<p>' . get_string('nodeadlines', 'theme_ccot') . '</p>';
        }
        return $o;
    }
    ...
    public function feature_spot_cards() {
        ...
        foreach ($fsnames as $feature) {
            ...
            if (!empty($PAGE->theme->settings->$title) && !empty($PAGE->theme->settings->$text)) {
                ...
                if (!empty($PAGE->theme->settings->$image)) {
                    ...
                    $img = '<!--Card image-->
                    <img class="ccot-feature-image" src="' .$url. '" alt="" role="presentation">';
                }
        ...
        if ($fscount > 0) {
            ...
            if (!empty($PAGE->theme->settings->fs_heading)) {
                $fstitle = '<h2 class="ccot-feature-spots-heading">' .s($PAGE->theme->settings->fs_heading). '</h2>';
            }
            ...
            foreach ($features as $feature) {
                $cards .= '<div class="' .$colclass. '" id="ccot-feature-' .$i. '">' .$feature. '</div>';
                $i++;
            }

            $fsedit = '';
            if ($this->page->user_is_editing()) {
                $url = new moodle_url('/admin/settings.php?section=themesettingccot#themeccotfeaturespots');
                $link = html_writer::link($url, get_string('featurespotsedit', 'theme_ccot'), ['class' => 'btn btn-primary']);
                $fsedit = '<p class="text-center">'.$link.'</p>';
            }

            // Build feature spots.
            $featurespots = '<div id="ccot-feature-spots">';
    ...
    protected function feature_spot_card($title, $image, $text) {
        $card = '<div class="ccot-feature">
            <!--Card content-->
            <div class="ccot-feature-block">
                ' .$image. '
                <!--Title-->
                <h3 class="ccot-feature-title h5">' .s($title). '</h3>
                <!--Content-->
                <p class="ccot-feature-text">' .s($text). '</p>
            </div>
            <!--/.Card content-->
        </div>';
        return $card;
    ...
    public function render_featured_courses(featured_courses $fc) {
        ...
        return $this->render_from_template('theme_ccot/featured_courses', $fc);
    }

    /**
     * Return ccot modchooser modal.
     * @return string
     */
    protected function course_modchooser() {
        ...
        return $this->render_from_template('theme_ccot/course_modchooser_modal', $data);
    }
    ...
    /**
     * Return ccot's logo url for login.mustache
     *
     * @param int $maxwidth not used in ccot.
     * @param int $maxheight not used in ccot.
    ...
    public function get_logo_url($maxwidth = null, $maxheight = 200) {
        ...
        $url = moodle_url::make_file_url("$CFG->httpswwwroot/pluginfile.php", "/$syscontextid/theme_ccot/logo/$itemid".$filepath);
        return $url;
    }
    ...
    protected function render_intelliboard() {
        ...
        $o .= '<div id="ccot-personal-menu-intelliboard">'
                .$links.
                '</div>';
    ...
    protected function render_intellicart() {
        ...
        $o .= '<div id="ccot-personal-menu-intellicart">'
            .$link.
            '</div>';
    ...
    public function navbar($coverimage = '') {
        ...
        $breadcrumbs = '';
        $courseitem = null;
        $attr['class'] = 'js-ccot-pm-trigger';

        if (!empty($coverimage)) {
            $attr['class'] .= ' mast-breadcrumb';
        }
        $ccotmycourses = html_writer::link('#', get_string('menu', 'theme_ccot'), $attr);
        ...
        foreach ($this->page->navbar->get_items() as $item) {
            ...
            // Replace my courses none-link with link to ccot personal menu.
            if ($item->key === 'mycourses') {
                $breadcrumbs .= '<li class="breadcrumb-item">' .$ccotmycourses. '</li>';
                continue;
            }
                ...
                if ($courseitem != null) {
                    ...
                    if ($COURSE->format == 'topics' || $COURSE->format == 'weeks') {
                        ...
                        if ($item->text == get_string('general')) {
                            $item->text = get_string('introduction', 'theme_ccot');
                        }
                    } else {
