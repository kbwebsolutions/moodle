...
namespace theme_ccot\output\core;
...
class course_renderer extends \core_course_renderer {
    ...
    $displayoptions = array()
        ...
        if ($modulehtml = $this->course_section_cm($course, $completioninfo, $mod, $sectionreturn, $displayoptions)) {
            list($ccotmodtype, $extension) = $this->get_mod_type($mod);

            if ($mod->modname === 'resource') {
                // Default for resources/attatchments e.g. pdf, doc, etc.
                if ($mod->showdescription == '1' || !empty($mod->availability)) {
                    $modclasses = array('ccot-extended-resource', 'ccot-mime-'.$extension);
                } else {
                    $modclasses = array('ccot-resource', 'ccot-mime-'.$extension);
                }
                if (in_array($extension, $this->ccot_multimedia())) {
                    $modclasses[] = 'js-ccot-media';
                }
                // For images we overwrite with the native class.
                if ($this->is_image_mod($mod)) {
                    $modclasses = array('ccot-native-image', 'ccot-image', 'ccot-mime-'.$extension);
                }
            } else if ($mod->modname === 'folder' && !$mod->url) {
                // Folder mod set to display on page.
                $modclasses = array('ccot-activity');
                ...
                if ((!empty($mod->showdescription) && $mod->showdescription == '1') || !empty($mod->availability)) {
                    $modclasses = array('ccot-extended-resource');
                } else {
                    $modclasses = array('ccot-resource');
                }
            ...
            if (in_array($mod->modname, ['page', 'book'])) {
                $modclasses = array('ccot-native', 'ccot-mime-'.$mod->modname);
                $attr['aria-expanded'] = "false";
            } else if ($modurl = $mod->url) {
                // For ccot cards, js uses this to make the whole card clickable.
                if ($mod->uservisible) {
            ...
            if (has_capability('moodle/course:update', $mod->context)) {
                $modclasses [] = 'ccot-can-edit';
            }

            $modclasses [] = 'ccot-asset'; // Added to stop conflicts in flexpage.
    ...
    public function course_section_cm($course, &$completioninfo, cm_info $mod, $sectionreturn, $displayoptions = array()) {
        ...
        // Drop section notice.
        if (has_capability('moodle/course:update', $mod->context)) {
            $output .= '<a class="ccot-move-note" href="#">'.get_string('movehere', 'theme_ccot').'</a>';
        }
        ...
        if (!empty($cmname)) {
            ...
            $ccotmodtype = $this->get_mod_type($mod)[0];
            $assetlink = '<div class="ccot-assettype">'.$ccotmodtype.'</div>';
            // Asset link.
            $assetlink .= '<h4 class="ccot-asset-link">'.$cmname.'</h4>';
        }
        ...
        // Due date, feedback available and all the nice ccot things.
        $ccotcompletionmeta = '';
        $ccotcompletiondata = $this->module_meta_html($mod);
        if ($ccotcompletiondata) {
            $ccotcompletionmeta = '<div class="ccot-completion-meta">'.$ccotcompletiondata.'</div>';
        }

        // Completion tracking.
        $completiontracking = '<div class="ccot-asset-completion-tracking">';
        ...
        if ($mod->is_stealth()) {
            // Stealth tag.
            $stealthtag = '<div class="ccot-stealth-tag">'.get_string('hiddenoncoursepage', 'moodle').'</div>';
        } else {
            // Draft status - always output, shown via css of parent.
            $drafttag = '<div class="ccot-draft-tag">'.get_string('draft', 'theme_ccot').'</div>';
        }
        ...
        if ($mod->modname !== 'resource') {
            ...
            if ($canmanagegroups && $mod->effectivegroupmode != NOGROUPS) {
                ...
                $groupmeta .= '<div class="ccot-group-tag">'.$groupinfo.'</div>';
            }
            ...
            if ($canmanagegroups && !empty($mod->groupingid)) {
                ...
                $groupmeta .= '<div class="ccot-grouping-tag">'.format_string($groupings[$mod->groupingid]->name).'</div>';
            }
        }
        ...
        if (!$mod->available || $canviewhidden) {
            $availabilityinfo = $this->course_section_cm_availability($mod, $displayoptions);
            if ($availabilityinfo) {
                $conditionalmeta .= '<div class="ccot-conditional-tag">'.$availabilityinfo.'</div>';
            }
        }
        ...
        // Build output.
        $postcontent = '<div class="ccot-asset-meta" data-cmid="'.$mod->id.'">'.$assetmeta.$mod->afterlink.'</div>';
        $output .= $assetlink.$postcontent.$contentpart.$ccotcompletionmeta.$groupmeta.$completiontracking;
        ...
        // TODO - add ccot strings here.

        // Move, Edit, Delete.
        if (has_capability('moodle/course:manageactivities', $modcontext)) {
            $movealt = s(get_string('move', 'theme_ccot', $mod->get_formatted_name()));
            $moveicon = '<img title="'.$movealt.'" aria-hidden="true" class="svg-icon" src="';
            $moveicon .= $this->output->image_url('move', 'theme').'"/>';
            $editalt = s(get_string('edit', 'theme_ccot', $mod->get_formatted_name()));
            $editicon = '<img title="'.$editalt.'" alt="'.$editalt.'" class="svg-icon" src="';
            $editicon .= $this->output->image_url('edit', 'theme').'"/>';
            $actions .= '<input id="ccot-move-mod-'.$mod->id.'" class="js-ccot-asset-move sr-only" type="checkbox">';
            $actions .= '<label class="ccot-asset-move" for="ccot-move-mod-'.$mod->id.'">';
            $actions .= '<span class="sr-only">'.$movealt.'</span>'.$moveicon.'</label>';
            $actions .= '<a class="ccot-edit-asset" href="'.new moodle_url($baseurl, array('update' => $mod->id)).'">';
            $actions .= $editicon.'</a>';
            $actionsadvanced[] = '<a href="'.new moodle_url($baseurl, array('delete' => $mod->id)).
                '" data-action="delete" class="js_ccot_delete dropdown-item">'.$str->delete.'</a>';
        }
        ...
        if (has_capability('moodle/course:activityvisibility', $modcontext) && !$mod->is_stealth()) {
            ...
            $hideaction .= '" data-action="hide" class="dropdown-item editing_hide js_ccot_hide">'.$str->hide.'</a>';
            $actionsadvanced[] = $hideaction;
            $showaction = '<a href="'.new moodle_url($baseurl, array('show' => $mod->id));
            $showaction .= '" data-action="show" class="dropdown-item editing_show js_ccot_show">'.$str->show.'</a>';
        ...
        if (has_all_capabilities($dupecaps, $coursecontext) &&
            ...
            $actionsadvanced[] = "<a href='".new moodle_url($baseurl, array('duplicate' => $mod->id)).
                "' data-action='duplicate' class='dropdown-item js_ccot_duplicate'>$str->duplicate</a>";
        }
        ...
        if (!empty($actionsadvanced)) {
            $moreicon = "<img title='".get_string('more', 'theme_ccot')."' alt='".get_string('more', 'theme_ccot').
                    "' class='svg-icon' src='".$this->output->image_url('more', 'theme')."'/>";
            $advancedactions = '<div class="dropdown ccot-edit-more-dropdown">';
            $advancedactions .= '<a href="#" class="dropdown-toggle ccot-edit-asset-more" ';
        ...
        // Add actions menu.
        if ($actions) {
            $output .= "<div class='js-only ccot-asset-actions' role='region' aria-label='actions'>";
    ...
    public function course_section_cm_text(cm_info $mod, $displayoptions = array()) {
        ...
        // Get custom module content for ccot, or get modules own content.
    ...
    */
    ...
    protected function get_mod_type(cm_info $mod) {
        if ($mod->modname === 'resource') {
            ...
            $extension = array(
                ...
                'archive' => 'zip',
                'pdf' => 'pdf',
                'image' => get_string('image', 'theme_ccot'),
            );
    ...
    protected function module_meta_html(cm_info $mod) {
        ...
        if ($meta->isteacher) {
            ...
            if (!$meta->submissionnotrequired && $meta->numsubmissions !== false) {
                $engagementmeta[] = get_string('xofy'.$meta->submitstrkey, 'theme_ccot',
                    (object) array(
                        'completed' => $meta->numsubmissions,
                        'participants' => \theme_ccot\local::course_participant_count($COURSE->id, $mod->modname)
                    )
                );
            }

            if ($meta->numrequiregrading) {
                $engagementmeta[] = get_string('xungraded', 'theme_ccot', $meta->numrequiregrading);
            }
            ...
            if (!empty($meta->grade)) {
                ...
                // theme/ccot/activity - for now its just hard coded.
                ...
                $feedbackavailable = get_string('feedbackavailable', 'theme_ccot');
    ...
    protected function mod_image_html($mod) {
        ...
        $icon = '<img title="' .get_string('vieworiginalimage', 'theme_ccot'). '"
                alt="' .get_string('vieworiginalimage', 'theme_ccot'). '"
                src="' .$OUTPUT->image_url('arrow-expand', 'theme'). '">';
        $imglink = '<a class="ccot-expand-link" href="' .$imgsrc. '" target="_blank">' .$icon. '</a>';

        $output = '<figure class="ccot-resource-figure figure">'
                    .$img.$imglink.
                    '<figcaption class="ccot-resource-figure-caption figure-caption">'
                        .$modname.$summary.
                    '</figcaption>
                </figure>';
    ...
    protected function mod_page_html(cm_info $mod) {
        ...
        $page = \theme_ccot\local::get_page_mod($mod);

        $imgarr = \theme_ccot\local::extract_first_image($page->content);
        ...
        $readmore = get_string('readmore', 'theme_ccot');
        ...
        <div class=pagemod-content tabindex='-1' data-content-loaded={$contentloaded}>
            {$content}
            <div class='d-block'><hr><a  class='ccot-action-icon ccot-icon-close' href='#'>
    ...
    public function book_get_toc($chapters, $book, $cm) {
        ...
        $toc = "<h6>".get_string('chapters', 'theme_ccot')."</h6>";
     ...
     * @return array
     */
    protected function ccot_multimedia() {
    ...
    public function course_section_cm_name(cm_info $mod, $displayoptions = array()) {
        ...
        $ccotmultimedia = $this->ccot_multimedia();

        if ($mod->modname === 'resource') {
            $extension = $this->get_mod_type($mod)[1];
            if (in_array($extension, $ccotmultimedia) ) {
     ...
     */
    public function ccot_footer_alert() {
        global $OUTPUT;
        return $OUTPUT->render_from_template('theme_ccot/footer_alert', null);
    }
    ...
    public function course_format_warning() {
        ...
        return $OUTPUT->notification(get_string('courseformatnotification', 'theme_ccot', $url->out()));
    }
    ...
    public function course_search_form($value = '', $format = 'plain') {
        ...
        return $this->render_from_template('theme_ccot/course_search_form', $data);
    }
    ...
    public function course_category($category) {
        ...
        $output = '';
        $categoryselector = '';
        // NOTE - we output manage catagory button in the layout file in ccot.
    ...
    public function course_footer() {
        ...
        if (!empty($teachers)) {
            ...
            // Course contacts.
            $courseteachers .= '<h5>'.get_string('coursecontacts', 'theme_ccot').'</h5>';
            foreach ($teachers as $teacher) {
        ...
        if (!empty($COURSE->summary)) {
            $coursesummary = '<h5>'.get_string('aboutcourse', 'theme_ccot').'</h5>';
            $formatoptions = new stdClass;
            $formatoptions->noclean = true;
            $formatoptions->overflowdiv = true;
            $formatoptions->context = $context;
            ...
            $coursesummary .= '<div id="ccot-course-footer-summary">'.$coursesummarycontent.'</div>';
        }
        ...
        if (empty($columns)) {
            ...
            $output .= '<div class="row">';
            $output .= '<div class="col-lg-3 col-md-4"><div id="ccot-course-footer-contacts">'.$courseteachers.'</div></div>';
            $output .= '<div class="col-lg-9 col-md-8"><div id="ccot-course-footer-about">'.$coursesummary.'</div></div>';
            $output .= '<div class="col-sm-12"><div id="ccot-course-footer-recent-activity">'.$courserecentactivity.'</div></div>';
            $output .= '</div>';
        }
        return $output;
    }
    ...
    public function get_mod_recent_activity($context) {
        ...
        if (!empty($recentactivity)) {
            foreach ($recentactivity as $modname => $moduleactivity) {
                ...
                $output .= $this->render_from_template('theme_ccot/media_object', $data);
            }
        }
        return $output;
    }
