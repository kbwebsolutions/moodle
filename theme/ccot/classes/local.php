...
namespace theme_ccot;

defined('MOODLE_INTERNAL') || die();

use \theme_ccot\user_forums,
    \theme_ccot\course_total_grade,
    html_writer,
    cm_info;
...
class local {
     ...
     * It's used by config.php so that hacks can be targetted at just the ccot theme.
    ...
    protected static function get_cachestamp($key, $cache, $new = false) {
        $key = strval($key);
        $muc = \cache::make('theme_ccot', $cache);
    ...
    public static function course_completion_progress($course) {
        ...
        $muc = \cache::make('theme_ccot', 'course_completion_progress');
    ...
    public static function courseinfo($courseids) {
        ...
        $showgrades = get_config('theme_ccot', 'showcoursegradepersonalmenu');
    ...
    public static function messages() {
        ...
        if (empty($messages)) {
            return '<p class="small">' . get_string('nomessages', 'theme_ccot') . '</p>';
        }
        ...
        foreach ($messages as $message) {
            ...
            $unreadclass = '';
            if ($message->unread) {
                $unreadclass = ' ccot-unread';
                $meta .= " <span class=ccot-unread-marker>".get_string('unread', 'theme_ccot')."</span>";
            }

            $info = '<p>'.format_string($message->smallmessage).'</p>';

            $o .= $output->ccot_media_object($url, $frompicture, $fromname, $meta, $info, $unreadclass);
        }
        return $o;
    }
    ...
    public static function graded($onlyactive = true) {
        ...
        foreach ($grades as $grade) {
            ...
            $meta = get_string('released', 'theme_ccot', $output->friendly_datetime($releasedon));

            $grade = new \grade_grade(array('itemid' => $grade->itemid, 'userid' => $USER->id));
            if (!$grade->is_hidden() || $canviewhiddengrade) {
                $o .= $output->ccot_media_object($url, $modimage, $gradetitle, $meta, '');
            }
        }

        if (empty($o)) {
            return '<p class="small">'. get_string('nograded', 'theme_ccot') . '</p>';
        }
        return $o;
    }
    ...
    public static function grading() {
        ...
        foreach ($grading as $key => $ungraded) {
            ...
            $xungraded = get_string('xungraded', 'theme_ccot', $ungraded->ungraded);

            $function = '\theme_ccot\activity::'.$cm->modname.'_num_submissions';
            ...
            $xofysubmitted = get_string('xofysubmitted', 'theme_ccot', $a);
            ...
            $out .= $output->ccot_media_object($cm->url, $modimage, $ungradedtitle, $meta, '');
        }

        if (empty($grading)) {
            return '<p class="small">' . get_string('nograding', 'theme_ccot') . '</p>';
        }
    ...
    /**
     * Make url based on file for theme_ccot components only.
     *
     * @param stored_file $file
     * @return \moodle_url | bool
     */
    private static function ccot_pluginfile_url($file) {
        if (!$file) {
            return false;
        } else {
    ...
    public static function course_card_clean_up($context) {
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'theme_ccot', 'coursecard');
    }
    ...
    public static function set_course_card_image($context, $originalfile) {
        if ($originalfile) {
            ...
            $fs = get_file_storage();
            $cardimage = $fs->get_file($context->id, 'theme_ccot', 'coursecard', 0, '/', 'course-card-'.$id.'-'.$filename);
            if ($cardimage) {
                return $cardimage;
            }
            $filespec = array(
                'contextid' => $context->id,
                'component' => 'theme_ccot',
                'filearea' => 'coursecard',
                'itemid' => 0,
                'filepath' => '/',
    ...
    public static function course_card_image_url($courseid) {
        $context = \context_course::instance($courseid);
        $fs = get_file_storage();
        $cardimages = $fs->get_area_files($context->id, 'theme_ccot', 'coursecard', 0, "itemid, filepath, filename", false);
        if ($cardimages) {
            ...
            if (stripos($cardimage->get_filename(), 'rawcoverimage.') !== false) {
                ...
                return self::ccot_pluginfile_url($cardimage);
            }
        }
    ...
    public static function coverimage($context) {
        ...
        $files = $fs->get_area_files($contextid, 'theme_ccot', 'coverimage', 0, "itemid, filepath, filename", false);
        if (!$files) {
            return false;
        }
        if (count($files) > 1) {
    ...
    public static function course_cat_coverimage_url($catid) {
        ...
        return self::ccot_pluginfile_url($file);
    }
    ...
    public static function site_coverimage_original() {
        ...
        if ($filename) {
            ...
            $fullpath = '/'.$syscontextid.'/theme_ccot/poster/0'.$filename;
            $fs = get_file_storage();
    ...
    public static function process_coverimage(\context $context, $originalfile = false) {
        ...
        $fs = get_file_storage();
        $fs->delete_area_files($context->id, 'theme_ccot', 'coverimage');
    ...
    public static function render_recent_forum_activity() {
        ...
        if (empty($activities)) {
            return '<p class="small">' . get_string('noforumposts', 'theme_ccot') . '</p>';
        }
        $activities = array_slice($activities, 0, 10);
        $renderer = $PAGE->get_renderer('theme_ccot', 'core', RENDERER_TARGET_GENERAL);
    ...
    public static function add_calendar_change_stamp($courseid) {
        $muc = \cache::make('theme_ccot', 'generalstaticappcache');
