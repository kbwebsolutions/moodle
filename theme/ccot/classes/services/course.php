...
class course {
    ...
    public function setcoverimage(\context $context, $data, $filename) {
        ...
        if (!in_array($ext, local::supported_coverimage_types())) {
            return ['success' => false, 'warning' => get_string('unsupportedcoverimagetype', 'theme_ccot', $ext)];
        }
        ...
        if (strlen($binary) > get_max_upload_file_size($CFG->maxbytes)) {
            throw new \moodle_exception('error:coverimageexceedsmaxbytes', 'theme_ccot');
        }
        ...
        if ($context->contextlevel === CONTEXT_COURSE) {
            ...
            if (!$this->check_summary_files_for_image_suitability($context)) {
                return ['success' => false, 'warning' => get_string('coursesummaryfilesunsuitable', 'theme_ccot')];
            }
            ...
            $fs->delete_area_files($context->id, 'theme_ccot', 'poster');
        } else {
        ...
        if ($context->contextlevel === CONTEXT_SYSTEM) {
            set_config('poster', $newfilename, 'theme_ccot');
    ...
    public function favorites($userid = null, $fromcache = true) {
        ...
        if (!isset($favorites[$userid])) {
            $favorites[$userid] = $DB->get_records('theme_ccot_course_favorites',
                ['userid' => $userid],
                'courseid ASC',
                'courseid'
            );
    ...
    public function setfavorite($courseshortname, $on = true, $userid = null) {
        ...
        if ($on) {
            if (!$favorited) {
                ...
                $DB->insert_record('theme_ccot_course_favorites', $data);
            }
        } else {
            if ($favorited) {
                $select = [
                ...
                $DB->delete_records('theme_ccot_course_favorites', $select);
            }
        }
    ...
    public function highlight_section($shortname, $sectionnumber, $highlight) {
        ...
        $actionmodel = new \theme_ccot\renderables\course_action_section_highlight($course, $section);
        $toc = new \theme_ccot\renderables\course_toc($course);
        return [
    ...
    public function set_section_visibility($shortname, $sectionnumber, $visible) {
        ...
        $modinfo = get_fast_modinfo($course);
        $section = $modinfo->get_section_info($sectionnumber);
        $actionmodel = new \theme_ccot\renderables\course_action_section_visibility($course, $section);
        $toc = new \theme_ccot\renderables\course_toc($course);
