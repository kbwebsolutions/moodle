...
$PAGE->set_url('/theme/ccot/rest.php', array('action' => $action, 'contextid' => $context->id));
...
foreach ($contfiles as $contfile) {
    ...
    if (preg_match($pattern, $contfile) !== 1) {
        ...
        $classname = '\\theme_ccot\\controller\\'.str_ireplace('.php', '', $contfile);
        if (class_exists($classname)) {
            $rc = new ReflectionClass($classname);
            if ($rc->isSubclassOf('\\theme_ccot\\controller\\controller_abstract')) {
