 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_theme_mypd extends behat_base {
    ...
    protected function upload_file($fixturefilename, $selector) {
        ...
        $filepath = $CFG->dirroot.'/theme/mypd/tests/fixtures/'.$fixturefilename;
        $file = $this->find('css', $selector);
        $file->attachFile($filepath);
    }

    /**
     * @param string $fixturefilename this is a filename relative to the mypd fixtures folder.
     * @param int $section
     *
    ...
    public function i_upload_file($fixturefilename, $section = 1) {
        $this->upload_file($fixturefilename, '#mypd-drop-file-'.$section);
    }
    ...
    public function i_upload_cover_image($fixturefilename) {
        $this->upload_file($fixturefilename, '#mypd-coverfiles');
        $this->getSession()->executeScript('jQuery( "#mypd-coverfiles" ).trigger( "change" );');
    }
    ...
    public function i_close_the_personal_menu() {
        ...
        if ($node->isVisible()) {
            $node = $this->find('css', '#mypd-pm-close');
            $node->click();
        }
    }
    ...
    public function i_see_availabilityinfo($str, $baseselector = '') {
        $str = trim($str);
        $nodes = $this->find_all('xpath', $baseselector.'//div[contains(@class, \'mypd-conditional-tag\')]');
    ...
    private function base_selector_availabilityinfo($type, $elementstr) {
        if ($type === 'section') {
            ...
            $baseselector = '(//li[contains(@class, \'mypd-asset\')]'. // Selection when editing teacher.
                '//h4[contains(@class, \'mypd-asset-link\')]'.
                '//span[contains(text(), \''.$elementstr.'\')]'.
                '/parent::a/parent::h4/parent::div'.
                '|'.
                '//li[contains(@class, \'mypd-asset\')]'. // Selection when anyone else.
                '//h4[contains(@class, \'mypd-asset-link\')]'.
    ...
    public function i_should_see_available_from_in_asset($date, $nthasset, $section) {
        ...
        $elementselector = '#section-'.$section.' li.mypd-asset:nth-of-type('.$nthasset.')';
    ...
    public function i_should_see_available_from_in_section($date, $section) {
        $elementselector = '#section-'.$section.' > div.content > .mypd-conditional-tag';
        return $this->i_should_see_available_from_in_element($date, $elementselector, 'css_element');
    }
    ...
    public function course_is_favorited($shortname) {
        ...
        /* @var behat_general $general */
        $general = behat_context_helper::get('behat_general');
        $this->ensure_element_does_not_exist('.mypd-icon-toggle.favoritetoggle.ajaxing', 'css_element');
    ...
    public function i_follow_in_the_mobile_menu($link) {
        $node = $this->get_node_in_container('link', $link, 'css_element', '#mypd-pm-mobilemenu');
    ...
    public function the_course_format_is_set_to($shortname, $format) {
        global $DB;
        $service = theme_mypd\services\course::service();
    ...
    public function i_highlight_section($section) {
        $xpath = '//li[@id="section-'.$section.'"]//div[contains(@class, "mypd-section-editing")]';
        $xpath .= '//a[contains(@class, "mypd-highlight")][@aria-pressed="false"]';
        $this->execute('behat_general::i_click_on', [$xpath, 'xpath_element']);
    }
    ...
    private function personal_menu_deadline_xpath($deadline, $eventname) {
        ...
        $xpath = "//div[@id='mypd-personal-menu-deadlines']//h3[contains(text(), '$eventname')]/parent::a/parent::div".
    ...
    private function meta_assign_xpath($name) {
        ...
        "/parent::div//div[contains(@class, 'mypd-completion-meta')]";
        return $xpath;
    }
    ...
    public function meta_assign_overdue($name) {
        ...
        $xpath .= "/a[contains(@class, 'mypd-due-date')][contains(@class, 'tag-danger')]";
        $this->ensure_element_is_visible($xpath, 'xpath_element');
    }
    ...
    public function meta_assign_not_overdue($name) {
        $xpath = $this->meta_assign_xpath($name);
        $xpath .= "/a[contains(@class, 'mypd-due-date')][contains(@class, 'tag-success')]";
        $this->ensure_element_is_visible($xpath, 'xpath_element');
        $xpath = $this->meta_assign_xpath($name);
        $xpath .= "/a[contains(@class, 'mypd-due-date')][contains(@class, 'tag-danger')]";
        $this->ensure_element_does_not_exist($xpath, 'xpath_element');
