...
@theme @theme_mypd
Feature: When the moodle theme is set to myPD, cover image can be set for site and courses.
  ...
  Scenario: Editing teachers can change and delete course cover image.
    ...
    And I wait until "label[for=\"mypd-coverfiles\"]" "css_element" is visible
  ...
  Scenario: Admin user can change and delete site cover image.
    ...
    Then I should see cover image in page header
    # Test deleting cover image
    And I navigate to "Appearance > Themes > myPD" in site administration
