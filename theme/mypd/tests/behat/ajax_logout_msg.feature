...
@theme @theme_mypd
Feature: When the moodle theme is set to myPD, ajax failures due to log outs / expired sessions are reported correctly
  as session issues.
  ...
  Scenario: Teacher get's login status warning when trying to manage sections if logged out.
    ...
    And I log out via a separate window
    When I click on "#section-2 .mypd-visibility.mypd-hide" "css_element"
    Then "body#page-login-index" "css_element" should exist
    ...
    Then "#section-2" "css_element" should exist
    And I log out via a separate window
    And I click on "#section-2 .mypd-highlight" "css_element"
    Then "body#page-login-index" "css_element" should exist
    ...
    Then I should see "Moving \"Untitled Topic\"" in the "#mypd-footer-alert" "css_element"
  ...
  Scenario: Teacher get's login status warning when trying to manage assets if logged out.
    ...
    And I log out via a separate window
    When I click on ".mypd-activity[data-type='Assignment'] a.js_mypd_hide" "css_element"
    ...
    And I log out via a separate window
    And I wait until the page is ready
    When I click on ".mypd-activity[data-type='Assignment'] a.js_mypd_duplicate" "css_element"
    Then "body#page-login-index" "css_element" should exist
    ...
    And I click on ".mypd-activity.modtype_assign .mypd-asset-move img[title='Move \"Test assignment\"']" "css_element"
    Then I should see "Moving \"Test assignment\""
    And I log out via a separate window
    When I click on "li#section-1 li.mypd-drop.asset-drop div.asset-wrapper" "css_element"
    Then I should see "You are logged out"
  ...
  Scenario: User account is created with force password change on login enabled, personal menu shows a warning when
    ...
    And I open the personal menu
    Then I should see "You must change your password before using the personal menu." in the "#mypd-pm-content .force-pwd-warning" "css_element"
