...
namespace theme_mypd\tests\controller;

use theme_mypd\controller\controller_abstract;
use theme_mypd\controller\router;
...
class router_test extends \basic_testcase {
    public function test_route_action() {
        $controller1 = $this->createPartialMock('\theme_mypd\controller\controller_abstract',
            array('init', 'test_action', 'require_capability'));
        $controller2 = $this->createPartialMock('\theme_mypd\controller\controller_abstract',
            array('init', 'test_action', 'require_capability'));
    ...
    public function test_route_fail() {
        $controller = $this->createPartialMock('\theme_mypd\controller\controller_abstract', array('init', 'require_capability'));
        $router     = new router();
        $router->add_controller($controller);
        $router->route_action('test');
    }
