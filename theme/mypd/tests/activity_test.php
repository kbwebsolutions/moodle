...
use \theme_mypd\activity,
    \theme_mypd\mypd_base_test;

global $CFG;
require_once($CFG->dirroot . '/mod/assign/tests/base_test.php');

/**
 * Testing for theme/mypd/classes/activity.php
 *
 * @package  theme_mypd
 * @copyright  2017 Blackboard Inc.
 * @license  http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_mypd_acitvity_test extends mypd_base_test {
    ...
    public function test_get_calendar_activity_events() {
        ...
        $mypdevent = reset($events);
        $this->assertEquals($due, $mypdevent->timestart);
        $this->assertEquals($due, $mypdevent->timesort);

        // Test extension set.
        $this->extend_assign_deadline($assign->id, $student->id, $extension);
        // Test that the myPD calendar includes extensions to assignments.
        $events = activity::get_calendar_activity_events($tstart, $tend, $courses);
        $mypdevent = reset($events);
        $this->assertEquals($extension, $mypdevent->timestart);
        $this->assertEquals($extension, $mypdevent->timesort);
        ...
        // Test that the myPD calendar event is the same as the core event
        // (with the exception of the timestart + timesort fields).
        unset($mypdevent->timestart);
        unset($mypdevent->timesort);
        unset($coreevent->timestart);
        unset($coreevent->timesort);
        $this->assertEquals($mypdevent, $coreevent);
    }
    ...
    public function test_user_activity_events() {
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovdgroupdue, $mypdevent->timestart);
        $this->assertEquals($ovdgroupdue, $mypdevent->timesort);
        ...
        // Assert not from cache.
        $this->assertFalse($eventsobj->fromcache);
        $this->assertEquals($ovduserdue, $mypdevent->timestart);
        $this->assertEquals($ovduserdue, $mypdevent->timesort);
    ...
    public function test_instance_activity_dates_quiz_assign() {
        ...
        $this->setUser($student);
        $quizdates = \theme_mypd\activity::instance_activity_dates($course->id, $cm, '', '');
        ...
        $dates1 = \theme_mypd\activity::instance_activity_dates($course->id, $cm, '', '');
        $dates2 = \theme_mypd\activity::instance_activity_dates($course->id, $cm2, '', '');
        ...
        $this->setUser($student);
        $assigndates = \theme_mypd\activity::instance_activity_dates($course->id, $cm, '', '');
