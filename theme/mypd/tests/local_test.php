...
namespace theme_mypd\tests;

use theme_mypd\local;
use theme_mypd\mypd_base_test;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_mypd_local_test extends mypd_base_test {
     ...
     * myPD theme settings page. Creates a file, sets a theme
    ...
    protected function fake_site_image_setting_upload($filename) {
        ...
        $filepath = $CFG->dirroot.'/theme/mypd/tests/fixtures/'.$filename;

        $fs = \get_file_storage();

        $fs->delete_area_files($syscontext->id, 'theme_mypd', 'poster');

        $fs->create_file_from_pathname($filerecord, $filepath);
        \set_config('poster', $filename, 'theme_mypd');
     ...
     * myPD theme settings page. Deletes a file, sets a theme
    ...
    protected function fake_site_image_setting_cleared() {
        ...
        $fs->delete_area_files($syscontext->id, 'theme_mypd', 'coverimage');

        \set_config('poster', '', 'theme_mypd');
        local::process_coverimage($syscontext);
    }
    ...
    public function test_poster_image_upload() {
        ...
        foreach ($fixtures as $filename => $shouldberesized) {
            ...
            $this->assertContains('/theme_mypd/coverimage/', $css);
    ...
    protected function fake_course_image_setting_upload($filename, $context) {
        ...
        $fs->create_file_from_pathname($filerecord, $filepath);
        \set_config('coverimage', $filename, 'theme_mypd');
    }
    ...
    public function test_course_grade() {
        ...
        set_config('showcoursegradepersonalmenu', 1, 'theme_mypd');
        ...
        set_config('showcoursegradepersonalmenu', 0, 'theme_mypd');
        $feedback = local::course_grade($course);
        $this->assertTrue(empty($feedback->coursegrade));
    }
    ...
    public function test_get_profile_based_branding() {
        ...
        \set_config('pbb_enable', '1', 'theme_mypd');
        ...
        $this->assertEquals('mypd-pbb-marketing', local::get_profile_based_branding_class($user));
        ...
        $this->assertEquals('mypd-pbb-super-enterprise-sales', local::get_profile_based_branding_class($user));
        ...
        \set_config('pbb_field', 'profile|' . $fieldid, 'theme_mypd');
        local::clean_profile_based_branding_cache();

        // Custom field is set as new value.
        $this->assertEquals('mypd-pbb-banana-split', local::get_profile_based_branding_class($user));
    }
}
