 ...
 */
define(['jquery', 'core/ajax', 'core/notification', 'core/log', 'theme_mypd/model_view', 'theme_mypd/ajax_notification'],
    function($, ajax, notification, log, mview, ajaxNotify) {
        return function() {
            ...
            var reloadCourseCardTemplate = function(renderable, cardEl) {
                var dfd = $.Deferred();
                mview(cardEl, 'theme_mypd/course_cards');
            ...
            var moveOutOfFavorites = function(cardEl, onMoveComplete) {
                ...
                // When this is 0 we only have hidden courses, so container is #mypd-pm-courses-current-cards.
                var publishedcount = $('#mypd-pm-courses-current .coursecard:not([data-hidden="true"])').length;
                // Special stuff for when moving a hidden course.
                if ($(cardEl).data('hidden') === true && publishedcount > 0) {
                    container = '#mypd-pm-courses-hidden-cards';
                    // Open hidden courses section.
                    $('#mypd-pm-courses-hidden').addClass('state-visible');
                    $('#mypd-pm-courses-hidden-cards').collapse('show');
                } else {
                    window.console.log('not a hidden card');
                    container = '#mypd-pm-courses-current-cards';
                }
            ...
            var favoriteCourse = function(button) {
                ...
                var doAjax = function(jsid) {
                    return ajax.call([
                        {
                            methodname: 'theme_mypd_course_card',
                ...
                if (favorited === 1) {
                    ...
                    // Move to favorites.
                    moveCard(cardEl, '#mypd-pm-courses-current-cards .coursecard.favorited', '#mypd-pm-courses-current-cards', true,
                        function() {
                            doAjax(jsid);
                        }
                    );
             ...
             */
            $("#mypd-pm").on("click", ".favoritetoggle", function(e) {
                e.preventDefault();
                e.stopPropagation();
                favoriteCourse(this);
            });
