...
define(
    ...
    function($, ajax, util, ajaxNotify, str) {
        ...
        var updateModCompletion = function(module, completionhtml) {
            ...
            module.find('.mypd-asset-completion-tracking').html(completionhtml);
            module.find('.btn-link').focus();
            $(document).trigger('mypdModuleCompletionChange', module);
        };
        ...
        var listenManualCompletion = function() {
            $('.course-content').on('submit', 'form.togglecompletion', function(e) {
                ...
                var module = $(form).parents('li.mypd-asset').first();
                form.addClass('ajaxing');

                ajax.call([
                    {
                        methodname: 'theme_mypd_course_module_completion',
        ...
        var listenPageModuleReadMore = function() {
            var pageToggleSelector = ".modtype_page .instancename,.pagemod-readmore,.pagemod-content .mypd-action-icon";
            $(document).on("click", pageToggleSelector, function(e) {
                ...
                if (pageModContent.data('content-loaded') == 1) {
                    ...
                    var readPageUrl = M.cfg.wwwroot + '/theme/mypd/rest.php?action=read_page&contextid=' +
                    ...
                    if (!isexpanded) {
                        ...
                        var loadingStrPromise = str.get_string('loading', 'theme_mypd');
                        ...
                        var getPageUrl = M.cfg.wwwroot + '/theme/mypd/rest.php?action=get_page&contextid=' +
                            readmore.data('pagemodcontext');
                        $.ajax({
                            type: "GET",
                            async: true,
        ...
        var lightboxMedia = function(resourcemod) {
            ...
            var lightbox = function(appendto, onclose) {
                var lbox = $('#mypd-light-box');
                if (lbox.length === 0) {
                    $(appendto).append('<div id="mypd-light-box" tabindex="-1">' +
                        '<div id="mypd-light-box-content"></div>' +
                        '<a id="mypd-light-box-close" class="pull-right mypd-action-icon mypd-icon-close" href="#">' +
                        '<small>Close</small>' +
                        '</a>' +
                        '</div>');
                    $('#mypd-light-box-close').click(function(e) {
                        e.preventDefault();
                        e.stopPropagation();
                        lightboxclose();
            ...
            var lightboxopen = function(content, appendto, onclose) {
                ...
                if (content) {
                    var contentdiv = $('#mypd-light-box-content');
                    contentdiv.html('');
            ...
            var spinner = '<div class="loadingstat three-quarters">' +
                Y.Escape.html(M.util.get_string('loading', 'theme_mypd')) +
                '</div>';
            ...
            $.ajax({
                type: "GET",
                async: true,
                url: M.cfg.wwwroot + '/theme/mypd/rest.php?action=get_media&contextid=' + $(resourcemod).data('modcontext'),
                success: function(data) {
                    ajaxNotify.ifErrorShowBestMsg(data).done(function(errorShown) {
                        if (errorShown) {
                            return;
        ...
        return {
            ...
            init: function() {
                ...
                $(document).on('click', '.js-mypd-media .mypd-asset-link [href*="/mod/resource/view.php?id="]', function(e) {
                    lightboxMedia($(this).closest('.mypd-resource, .mypd-extended-resource'));
                    e.preventDefault();
                });

                // Make resource cards clickable.
                $(document).on('click', '.mypd-resource-card .mypd-resource, .mypd-extended-resource', function(e) {
                    var trigger = $(e.target),
                        hreftarget = '_self',
                        link = $(trigger).closest('.mypd-resource').find('.mypd-asset-link a'),
                        href = '';
                    if (link.length > 0) {
                        href = $(link).attr('href');
                    }
                    ...
                    var selector = '.mypd-asset-completion-tracking, .mypd-asset-actions, .contentafterlink a, .ally-actions';
                    var withintarget = $(trigger).closest(selector).length;
                    if (!withintarget) {
                        if ($(this).hasClass('js-mypd-media')) {
                            lightboxMedia(this);
                        } else {
                            if (href === '') {
                                return;
