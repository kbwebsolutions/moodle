...
define(['jquery', 'core/log', 'core/ajax', 'core/notification', 'theme_mypd/ajax_notification'],
    function($, log, ajax, notification, ajaxNotify) {
        ...
        var addCoverImageAlert = function(id, msg) {
            ...
            if (!$(id).length) {
                $('#mypd-coverimagecontrol').before(
                    '<div id="' + id + '" class="mypd-alert-cover-image alert alert-warning" role="alert">' +
                    msg +
         ...
         */
        var state1 = function() {
            $('#mypd-changecoverimageconfirmation .ok').removeClass('ajaxing');
            $('#mypd-alert-cover-image-size').remove();
            $('#mypd-alert-cover-image-bytes').remove();
            $('label[for="mypd-coverfiles"]').removeClass('ajaxing');
            $('#mypd-changecoverimageconfirmation').removeClass('state-visible');
            $('label[for="mypd-coverfiles"]').addClass('state-visible');
            $('#mypd-coverfiles').val('');
        };
         ...
         */
        var state2 = function() {
            $('#mypd-alert-cover-image-upload-failed').remove();
            $('#mypd-changecoverimageconfirmation').removeClass('disabled');
            $('label[for="mypd-coverfiles"]').removeClass('state-visible');
            $('#mypd-changecoverimageconfirmation').addClass('state-visible');
        ...
        var coverImage = function(siteMaxBytes, ajaxParams) {
            ...
            $('#mypd-coverfiles').on('change', function(e) {
                ...
                $('label[for="mypd-coverfiles"]').addClass('ajaxing');
                ...
                reader.onload = (function(theFile) {
                    return function(e) {
                        ...
                        if (theFile.size > maxbytes) {
                            ...
                            var message = M.util.get_string('error:coverimageexceedsmaxbytes', 'theme_mypd', maxbytesstr);
                            addCoverImageAlert('mypd-alert-cover-image-bytes', message);
                            return;
                        } else {
                            $('#mypd-alert-cover-image-bytes').remove();
                        }
                        ...
                        $(img).on('load', function() {
                            if (img.width < 1024) {
                                addCoverImageAlert('mypd-alert-cover-image-size',
                                    M.util.get_string('error:coverimageresolutionlow', 'theme_mypd')
                                );
                            } else {
                                $('#mypd-alert-cover-image-size').remove();
                            }
            ...
            $('#mypd-changecoverimageconfirmation .ok').click(function() {
                ...
                $('#mypd-changecoverimageconfirmation .ok').addClass('ajaxing');
                $('#mypd-changecoverimageconfirmation').addClass('disabled');
                ...
                ajax.call([
                    {
                        methodname: 'theme_mypd_cover_image',
                        args: {params: ajaxParams},
                        done: function(response) {
                            state1();
                            if (!response.success && response.warning) {
                                addCoverImageAlert('mypd-alert-cover-image-upload-failed', response.warning);
                            }
                        },
            ...
            $('#mypd-changecoverimageconfirmation .cancel').click(function() {
            ...
            $('#mypd-coverimagecontrol').addClass('mypd-js-enabled');
        };
