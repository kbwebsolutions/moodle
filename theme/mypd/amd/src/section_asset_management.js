...
define(['jquery', 'core/log', 'core/ajax', 'core/str', 'core/templates', 'core/notification',
    'theme_mypd/util', 'theme_mypd/ajax_notification', 'theme_mypd/footer_alert'],
    ...
    return {
        init: function(courseLib) {
            ...
            var stopMoving = function() {
                $('body').removeClass('mypd-move-inprogress');
                $('body').removeClass('mypd-move-section');
                $('body').removeClass('mypd-move-asset');
                footerAlert.hideAndReset();
                $('.section-moving').removeClass('section-moving');
                $('.asset-moving').removeClass('asset-moving');
                $('.js-mypd-asset-move').removeAttr('checked');
                movingObjects = [];
            };
            ...
            var moveFailed = function() {
                ...
                footerAlert.setTitle(M.util.get_string('movefailed', 'theme_mypd', actname));
            ...
            var updateMovingMessage = function() {
                ...
                if (movingObjects.length === 1) {
                    var assetname = $(movingObjects[0]).find('.mypd-asset-link .instancename').html();
                    assetname = assetname || M.util.get_string('pluginname', 'label', assetname);
                    title = M.util.get_string('moving', 'theme_mypd', assetname);
                } else {
                    title = M.util.get_string('movingcount', 'theme_mypd', movingObjects.length);
                }
            ...
            var updateSectionNavigation = function(selector) {
                ...
                $.each(sections, function(idx, el) {
                    ...
                    templates.render('theme_mypd/course_section_navigation', navigation)
            ...
            var sectionDelete = function(e, el) {
                ...
                var doDelete = function() {
                    ...
                    var delProgress = M.util.get_string('deletingsection', 'theme_mypd', sectionName);
                    ...
                    var ajaxPromises = ajax.call([
                        {
                            methodname: 'theme_mypd_course_sections',
                            args: params
                        }
                    ], true, true);
                    ...
                    ajaxPromises[0]
                        .done(function(response) {
                            // Update TOC.
                            templates.render('theme_mypd/course_toc', response.toc)
                                .done(function(result) {
                                    $('#course-toc').html($(result).html());
                                    $(document).trigger('mypdTOCReplaced');
                ...
                var ok = M.util.get_string('deletesectionconfirm', 'theme_mypd');
            ...
            var assetAction = function(e, triggerEl) {
                ...
                var assetEl = $($(triggerEl).parents('.mypd-asset')[0]),
                ...
                var actionAJAX = function() {
                    if (!ajaxTracker.start(jsPendingKey, assetEl, '.mypd-edit-asset-more')) {
                        // Request already made.
                        return;
                    }
                ...
                var getErrorStrings = function() {
                    ...
                    return str.get_strings([
                        {key: errActionKey, component: 'theme_mypd'},
                        {key: errMessageKey, component: 'theme_mypd'}
                    ]);
                };
                ...
                getErrorStrings().then(function(strings) {
                    ...
                    if (action === 'delete') {
                        ...
                        var delTitle = M.util.get_string('confirm', 'moodle');
                        var ok = M.util.get_string('deleteassetconfirm', 'theme_mypd', plugindata.type);
                        var cancel = M.util.get_string('cancel', 'moodle');
            ...
            var ajaxReqMoveAsset = function(target) {
                ...
                if (target && !$(target).hasClass('mypd-drop')) {
            ...
            var ajaxReqMoveSection = function(dropzone) {
                ...
                ajaxReqMoveGeneral(params, function() {
                    ...
                    ajax.call([
                        {
                            methodname: 'theme_mypd_course_toc_chapters',
                            args: {
                                courseshortname: courseLib.courseConfig.shortname
                            },
                            done: function(response) {
                                // Update TOC.
                                templates.render('theme_mypd/course_toc_chapters', response.chapters)
                                    .done(function(result) {
                                        // Update chapters.
            ...
            var assetEditListeners = function() {
                var actionSelectors = '.mypd-asset-actions .js_mypd_hide, ';
                actionSelectors += '.mypd-asset-actions .js_mypd_show, ';
                actionSelectors += '.mypd-asset-actions .js_mypd_delete, ';
                actionSelectors += '.mypd-asset-actions .js_mypd_duplicate';
            ...
            var sectionActionListener = function(action, onComplete) {
                ...
                $('#region-main').on('click', '.mypd-section-editing.actions .mypd-' + action, function(e) {
                    ...
                    if (action === 'visibility') {
                        toggle = $(this).hasClass('mypd-hide') ? 0 : 1;
                    } else {
                    ...
                    var sectionActionsSelector = '#section-' + sectionNumber + ' .mypd-section-editing';
                    var actionSelector = sectionActionsSelector + ' .mypd-' + action;
                    ...
                    .fail(function(response) {
                        ...
                        if (action === 'visibility') {
                            errMessage = M.util.get_string('error:failedtochangesectionvisibility', 'theme_mypd');
                            errAction = M.util.get_string('action:changesectionvisibility', 'theme_mypd');
                        } else {
                            errMessage = M.util.get_string('error:failedtohighlightsection', 'theme_mypd');
                            errAction = M.util.get_string('action:highlightsectionvisibility', 'theme_mypd');
                        }
                        ajaxNotify.ifErrorShowBestMsg(response, errAction, errMessage).done(function() {
                            // Allow another request now this has finished.
                        ...
                        return templates.render('theme_mypd/course_action_section', response.actionmodel)
                        .then(function(result) {
                            $(actionSelector).replaceWith(result);
                            $(actionSelector).focus();
                            // Update TOC.
                            return templates.render('theme_mypd/course_toc', response.toc);
                        }).then(function(result) {
                            $('#course-toc').html($(result).html());
                            $(document).trigger('mypdTOCReplaced');
            ...
            var highlightSectionListener = function() {
                sectionActionListener('highlight', function(sectionNumber) {
                    ...
                    $notCurrent.each(function() {
                        var highlighter = $(this).find('.mypd-highlight');
            ...
            var deleteSectionListener = function() {
                $(document).on('click', '.mypd-section-editing.actions .mypd-delete', function(e) {
            ...
            var moveSectionListener = function() {
                ...
                $("#region-main").on('click', '.mypd-section-editing.actions .mypd-move', function(e) {
                    e.stopPropagation();
                    e.preventDefault();

                    $('body').addClass('mypd-move-inprogress');
                    footerAlertShowMove();
                    ...
                    $('body').addClass('mypd-move-section');

                    var title = M.util.get_string('moving', 'theme_mypd', sectionName);
                    footerAlert.setTitle(title);

                    $('.section-drop').each(function() {
                        var sectionDropMsg = M.util.get_string('movingdropsectionhelp', 'theme_mypd',
                            {moving: sectionName, before: $(this).data('title')}
                        );
                        $(this).html(sectionDropMsg);
                    });

                    footerAlert.setSrNotice(M.util.get_string('movingstartedhelp', 'theme_mypd', sectionName));
                });
            };
            ...
            var addAfterDrops = function() {
                if (document.body.id === "page-site-index") {
                    $('#region-main .sitetopic ul.section').append(
                        '<li class="mypd-drop asset-drop">' +
                        '<div class="asset-wrapper">' +
                        '<a href="#">' +
                        M.util.get_string('movehere', 'theme_mypd') +
                        '</a>' +
                        '</div>' +
                        '</li>');
                } else {
            ...
            var assetMoveListener = function() {
                $("#region-main").on('change', '.js-mypd-asset-move', function(e) {
                    e.stopPropagation();

                    var asset = $(this).parents('.mypd-asset')[0];

                    // Make sure after drop is at the end of section.
                    var section = $(asset).parents('ul.section')[0];
                    var afterdrop = $(section).find('li.mypd-drop.asset-drop');
                    $(section).append(afterdrop);

                    if (movingObjects.length === 0) {
                        // Moving asset - activity or resource.
                        // Initiate move.
                        var assetname = $(asset).find('.mypd-asset-link .instancename').html();

                        log.debug('Moving this asset', assetname);

                        var classes = $(asset).attr('class'),
                            regex = /(?=mypd-mime)([a-z0-9\-]*)/;
                        ...
                        $(asset).find('.js-mypd-asset-move').prop('checked', 'checked');

                        $('body').addClass('mypd-move-inprogress');
                        $('body').addClass('mypd-move-asset');
                    }
            ...
            var movePlaceListener = function() {
                $(document).on('click', '.mypd-move-note, .mypd-drop', function(e) {
                    log.debug('myPD drop clicked', e);
                    if (movingObjects) {
                        e.stopPropagation();
                        e.preventDefault();
                        if ($('body').hasClass('mypd-move-section')) {
                            ajaxReqMoveSection(this);
                        } else {
                            var target;
                            if ($(this).hasClass('mypd-drop')) {
                                target = this;
                            } else {
                                target = $(this).closest('.mypd-asset');
                            }
                            ajaxReqMoveAsset(target);
                        }
                    }
            ...
            var addListeners = function() {
                ...
                assetEditListeners();
                addAfterDrops();
                $('body').addClass('mypd-course-listening');
            };
