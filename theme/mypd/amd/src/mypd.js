...
/**
 * Main mypd initialising function.
 */
define(['jquery', 'core/ajax', 'core/log', 'theme_mypd/headroom', 'theme_mypd/util', 'theme_mypd/personal_menu',
        'theme_mypd/cover_image', 'theme_mypd/progressbar', 'core/templates', 'core/str'],
    function($, ajax, log, Headroom, util, personalMenu, coverImage, ProgressBar, templates, str) {

        'use strict';

        /* eslint-disable camelcase */
        M.theme_mypd = M.theme_mypd || {};
        ...
        var movePHPErrorsToHeader = function() {
            ...
            if (msgs.length) {
                ...
                var errorcont = $('<div id="footer-error-cont"><h3>' +
                    M.util.get_string('debugerrors', 'theme_mypd') +
                    '</h3><hr></div>');
                ...
                var errorlink = $('<a class="footer-error-link btn btn-danger" href="#footer-error-cont">' +
                    M.util.get_string('problemsfound', 'theme_mypd') + ' <span class="badge">' + (msgs.length) + '</span></a>');
        ...
        var bodyClasses = function() {
            var extraclasses = [];
            if ($('#notice.mypd-continue-cancel').length) {
            ...
            $(window).on('popstate hashchange', function(e) {
                ...
                if (newHash !== lastHash) {
                    if (location.hash === '#primary-nav') {
                        ...
                        $('#page, #moodle-footer, #js-mypd-pm-trigger, #logo, .skiplinks').css('display', '');
                        if (onCoursePage()) {
        ...
        var recentUpdatesFix = function() {
            $('#mypd-course-footer-recent-activity .info').each(function() {
                $(this).appendTo($(this).prev());
            });
            $('#mypd-course-footer-recent-activity .head .name').each(function() {
        ...
        var addListeners = function() {
            ...
            $(document).on("click", ".news-article .toggle", function(e) {
                ...
                $('.state-expanded').find('.news-article-message').slideDown("fast", function() {
                    ...
                    $(document).trigger('mypdContentRevealed');
                });
                e.preventDefault();
            });
        ...
        return {
             ...
             */
            mypdInit: function(courseConfig, pageHasCourseContent, siteMaxBytes, forcePassChange,
                               messageBadgeCountEnabled, userId, sitePolicyAcceptReqd, inAlternativeRole) {

                // Set up.
                M.cfg.context = courseConfig.contextid;
                M.mypdTheme = {forcePassChange: forcePassChange};
                ...
                $(document).ready(function() {
                    ...
                    // Allow deeplinking to bs tabs on mypd settings page.
                    if ($('#page-admin-setting-themesettingmypd').length) {
                    ...
                    if ((onModSettings || onCourseSettings || onSectionSettings) && pageNotInBlacklist) {
                        ...
                        $('#mform1 > fieldset').not(vital).wrapAll('<div class="mypd-form-advanced col-md-4" />');

                        // Add expand all to advanced column.
                        $(".mypd-form-advanced").append($(".collapsible-actions"));
                        // Add collapsed to all fieldsets in advanced, except on course edit page.
                        if (!$('#page-course-edit').length) {
                            $(".mypd-form-advanced fieldset").addClass('collapsed');
                        }
                        ...
                        $(mainForm).wrap('<div class="mypd-form-required col-md-8" />');
                        ...
                        $("#page-mod-resource-mod .mypd-form-advanced #id_modstandardelshdr .fcontainer").append(description);
                        $("#page-mod-resource-mod .mypd-form-advanced #id_modstandardelshdr .fcontainer").append(showdescription);
                        ...
                        $("#page-mod-assign-mod .mypd-form-advanced #id_modstandardelshdr .fcontainer").append(filemanager);
                        $("#page-mod-assign-mod .mypd-form-required .fcontainer").append(duedate);

                        // Move availablity at the top of advanced settings.
                        var availablity = $('#id_visible').closest('.form-group').addClass('mypd-form-visibility');
                        ...
                        $(label).text(M.util.get_string('visibility', 'theme_mypd') + ' ');
                        ...
                        if ($("#page-course-edit").length) {
                            ...
                            (function(){
                                return str.get_strings([
                                    {key: 'showallsectionsdisabled', component: 'theme_mypd'},
                                    {key: 'disabled', component: 'theme_mypd'}
                                ]);
                            })()
                                .then(function(strings) {
                                    var strMessage = strings[0];
                                    strDisabled = strings[1];
                                    return templates.render('theme_mypd/form_alert', {
                                        type: 'warning',
                                        classes: '',
                                        message: strMessage
                                    });
                        ...
                        $('.mypd-form-advanced').prepend(availablity);
                        ...
                        if ($("#page-mod-page-mod").length) {
                            ...
                            (function() {
                                return str.get_strings([
                                    {key: 'showappearancedisabled', component: 'theme_mypd'}
                                ]);
                            })()
                    ...
                    if (messageBadgeCountEnabled) {
                        require(
                            [
                                'theme_mypd/conversation_badge_count-lazy'
                    ...
                    var focustarget = $('#mypd-coverimagecontrol label');
                    if (focustarget && focustarget.length) {
                        focustarget.keypress(function (e) {
                            if (e.which === 13) {
                                $('#mypd-coverfiles').trigger('click');
                            }
                        });
                    }
                    ...
                    if (!$('.block_settings').length) {
                        ...
                        if (inAlternativeRole) {
                            ...
                            require(
                                [
                                    'theme_mypd/alternative_role_handler-lazy'
                    ...
                    var mypdHome = $('#mypd-home');
                    if (notificationsBtn.length && searchButton.length && adminMenuTrigger.length && mypdHome.length) {
                        // Update tab events because all elements have tabindex="0" and they are rendered funny.
                        require(
                            [
                                'theme_mypd/rearrange_tab_handler-lazy'
                            ], function(searchTabHandler) {
                                searchTabHandler.init([notificationsBtn, searchButton, adminMenuTrigger, mypdHome]);
                            }
                        );
                    }

                    // Add setttings tab show behaviour to classes which want to do that.
                    $('.mypd-settings-tab-link').on('click', function() {
                    ...
                    $('.dropdown-item.editing_hide.js_mypd_hide').click(function(e) {
                        e.stopPropagation();
                        $.ajax({
                            type: "GET",
