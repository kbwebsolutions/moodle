...
/**
 * Standard library functions for mypd theme.
 *
 * @package   theme_mypd
 ...
 */
function theme_mypd_process_site_coverimage() {
    $context = \context_system::instance();
    \theme_mypd\local::process_coverimage($context);
 ...
 * @return string
 */
function theme_mypd_process_css($css, theme_config $theme) {

    $css = theme_mypd_set_category_colors($css, $theme);

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_mypd_set_logo($css, $logo);
    ...
    $css = theme_mypd_set_customcss($css, $customcss);
 ...
 */
function theme_mypd_set_category_colors($css, $theme) {
    global $DB;
    ...
    $dbcategorycolors = get_config("theme_mypd", "category_color");
    ...
    if (!empty($categorycolors)) {
        ...
        $themedirectory = realpath(core_component::get_component_directory('theme_mypd'));
        ...
        foreach ($categories as $category) {
            ...
            $scss = str_replace('.theme-mypd {', $categoryselector, $brandscss);
 ...
 */
function theme_mypd_set_logo($css, $logo) {
    $tag = '/**setting:logo**/';
    if (is_null($logo)) {
        $replacement = '';
    } else {
        $replacement = "#mypd-home.logo, .mypd-logo-sitename {background-image: url($logo);}";
    }
    $css = str_replace($tag, $replacement, $css);
    return $css;
}
 ...
 */
function theme_mypd_set_customcss($css, $customcss) {
 ...
 * @return bool
 */
function theme_mypd_send_file($context, $filearea, $args, $forcedownload, $options) {
    ...
    $fullpath = "/$contextid/theme_mypd/$filearea/0/$filename";
    $fs = get_file_storage();
 ...
 * @return bool
 */
function theme_mypd_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    ...
    if ($context->contextlevel == CONTEXT_SYSTEM && in_array($filearea, $sysfileareas)) {
        $theme = theme_config::load('mypd');
        return $theme->setting_file_serve($filearea, $args, $forcedownload, $options);
    } else if (in_array($context->contextlevel, $coverimagecontexts)
        && $filearea == 'coverimage' || $filearea == 'coursecard') {
        theme_mypd_send_file($context, $filearea, $args, $forcedownload, $options);
    } else {
        send_file_not_found();
    }
}

function theme_mypd_myprofile_navigation(core_user\output\myprofile\tree $tree, $user, $iscurrentuser, $course) {
    global $PAGE;

    if ($PAGE->theme->name === 'mypd') {
        if ($iscurrentuser) {
...
function theme_mypd_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_mypd', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_mypd and not theme_boost (see the line above).
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/mypd/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    $post = file_get_contents($CFG->dirroot . '/theme/mypd/scss/post.scss');
 ...
 * @return array
 */
function theme_mypd_get_pre_scss($theme) {
    global $CFG;
