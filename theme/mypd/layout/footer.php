...
if ($this->page->user_is_editing() && $PAGE->pagetype == 'site-index') {
    $url = new moodle_url('/admin/settings.php', ['section' => 'themesettingmypd'], 'admin-footnote');
    $link = html_writer::link($url, get_string('editcustomfooter', 'theme_mypd'), ['class' => 'btn btn-primary btn-sm']);
...
if (!empty($custommenu) && $this->page->user_is_editing() && $PAGE->pagetype == 'site-index') {
    ...
    $link = html_writer::link($url, get_string('editcustommenu', 'theme_mypd'), ['class' => 'btn btn-primary btn-sm']);
    $custommenu .= '<p class="text-right">'.$link.'</p>';
}


/* myPD main footer. */
echo '<div id="mypd-site-footer">';
if (!empty($footnote)) {
    echo '<div id="mypd-footer-content">';
    echo $footnote;
    echo '</div>';
}
/* Social media links. */
...
if (!empty($socialmedialinks)) {
    echo '<div id="mypd-socialmedia-links">' .$socialmedialinks. '</div>';
}
echo '</div>';
?>
...
<div class="row">
    <div id="mrooms-footer" class="helplink col-sm-6">
        <small>
            ...
            echo get_string('poweredbyrunby', 'theme_mypd', date('Y', time()));
            ?>
        </small>
    </div>
