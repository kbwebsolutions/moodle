...
$PAGE->set_url('/theme/mypd/rest.php', array('action' => $action, 'contextid' => $context->id));
...
foreach ($contfiles as $contfile) {
    ...
    if (preg_match($pattern, $contfile) !== 1) {
        ...
        $classname = '\\theme_mypd\\controller\\'.str_ireplace('.php', '', $contfile);
        if (class_exists($classname)) {
            $rc = new ReflectionClass($classname);
            if ($rc->isSubclassOf('\\theme_mypd\\controller\\controller_abstract')) {
