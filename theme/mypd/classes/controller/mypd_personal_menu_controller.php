...
namespace theme_mypd\controller;
 ...
 */
class mypd_personal_menu_controller extends controller_abstract {
    /**
    ...
    public function get_deadlines_action() {
        ...
        $deadlines = \theme_mypd\activity::upcoming_deadlines($USER->id);
        return json_encode([
    ...
    public function get_forumposts_action() {
        return json_encode(array(
            'html' => \theme_mypd\local::render_recent_forum_activity()
        ));
    }
    ...
    public function get_courseinfo_action() {
        ...
        $courseinfo = \theme_mypd\local::courseinfo($courseids);
        return json_encode(array(
            'info' => $courseinfo
        ));
    }
    ...
    public function get_loginstatus_action() {
        ...
        if (!$loggedin) {
            if (!empty($failedactionmsg)) {
                $return['loggedoutmsg'] = get_string('loggedoutfailmsg', 'theme_mypd', $failedactionmsg);
            } else {
                $return['loggedoutmsg'] = get_string('loggedoutmsg', 'theme_mypd');
            }
            $return['loggedouttitle'] = get_string('loggedoutmsgtitle', 'theme_mypd');
