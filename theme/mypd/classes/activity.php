...
class activity {
    ...
    public static function module_meta(\cm_info $mod) {
        ...
        if (method_exists('theme_mypd\\activity', $methodname)) {
            $meta = call_user_func('theme_mypd\\activity::' . $methodname, $mod);
        } else {
    ...
    protected static function std_meta(\cm_info $mod,
        ...
        $meta->submittedstr = get_string($submitstrkey, 'theme_mypd');
        $meta->notsubmittedstr = get_string('not'.$submitstrkey, 'theme_mypd');
        if (get_string_manager()->string_exists($mod->modname.'draft', 'theme_mypd')) {
            $meta->draftstr = get_string($mod->modname.'draft', 'theme_mypd');
        } else {
            $meta->drafstr = get_string('draft', 'theme_mypd');
        }

        if (get_string_manager()->string_exists($mod->modname.'reopened', 'theme_mypd')) {
            $meta->reopenedstr = get_string($mod->modname.'reopened', 'theme_mypd');
        } else {
            $meta->reopenedstr = get_string('reopened', 'theme_mypd');
        }
        ...
        if (has_capability('mod/assign:grade', $mod->context)) {
            ...
            if (method_exists('theme_mypd\\activity', $methodnsubmissions)) {
                $meta->numsubmissions = call_user_func('theme_mypd\\activity::'.$methodnsubmissions, $courseid, $mod->instance);
            }
            if (method_exists('theme_mypd\\activity', $methodnungraded)) {
                $meta->numrequiregrading = call_user_func('theme_mypd\\activity::'.$methodnungraded, $courseid, $mod->instance);
            }
        } else {
...
-- Exclude those people who can grade quizzes
    ...
    public static function get_events(
        ...
        \theme_mypd\calendar\event\container::ovd_init();
        $vault = \theme_mypd\calendar\event\container::get_event_vault();
    ...
    public static function user_activity_events($userorid, array $courses, $tstart, $tend, $cacheprefix = '',
        ...
        if (self::$phpunitallowcaching || !(defined('PHPUNIT_TEST') && PHPUNIT_TEST)) {
            $muc = \cache::make('theme_mypd', 'activity_deadlines');
