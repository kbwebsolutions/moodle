...
class shared extends \renderer_base {
    ...
    protected static function dndupload_add_to_course($course, $modnames) {
        ...
        $script = <<<EOF
            <script>
                var thememyPDCourseFileHandlers = $json;
            </script>
EOF;
        ...
        $PAGE->requires->js_call_amd('theme_mypd/dndupload-lazy', 'init', $vars);
    }
    ...
    public static function page_requires_js() {
        ...
        $PAGE->requires->js_call_amd('theme_mypd/mypd', 'mypdInit', $initvars);
    ...
    public static function appendices() {
        ...
        foreach ($instances as $instance) { // Need to check enrolment methods for self enrol.
            if ($instance->enrol === 'self') {
                ...
                if (is_enrolled($coursecontext)) {
                    ...
                    if ($enrolurl) {
                        $selfenrol = true;
                        $enrolstr = get_string('unenrolme', 'theme_mypd');
                        break;
                    }
                } else {
        ...
        if (has_capability('moodle/course:update', $coursecontext)) {
            ...
            $settingsicon = '<img src="'.$iconurl.'" class="mypd-cover-icon svg-icon" alt="" role="presentation">';

            $links[] = array(
                'link' => 'course/edit.php?id='.$COURSE->id,
                'title' => $settingsicon.get_string('editcoursesettings', 'theme_mypd'),
            );
        }
        ...
        if (course_can_view_participants($coursecontext)) {
            ...
            $usercount = \theme_mypd\local::count_enrolled_users($coursecontext, '', 0, true);
            ...
            $participanticons = '<div class="mypd-participant-icons">'.$participanticons.'</div>';
            $links[] = array(
        ...
        if (array_key_exists('pld', $localplugins) && has_capability('local/pld:editcourserules', $coursecontext)) {
            ...
            $pldname = get_string('pld', 'theme_mypd');
            $links[] = array(
        ...
        $coursetools = get_string('coursetools', 'theme_mypd');
    ...
    public static function print_student_dashboard() {
        ...
        $userpicture->class = 'userpicture mypd-icon'; // Icon class for margin.
        $userpicture->size = 100;
        $userpic = $OUTPUT->render($userpicture);

        $userboard  = '<div id="mypd-student-dashboard" class="row clearfix">';
        ...
        if ($COURSE->enablecompletion) {
            ...
            $userboard .= '<div class="col-xs-3 text-center mypd-student-dashboard-progress">';
            $userboard .= '<h4 class="h6">' .get_string('progress', 'theme_mypd'). '</h6>';
            $userboard .= '<div class="js-progressbar-circle mypd-progress-circle" value="' .round($progress->progress). '"></div>';
            $userboard .= '</div>';
        }
        ...
        if (has_capability('gradereport/overview:view', $coursecontext)) {
            ...
            $userboard .= '<div class="col-xs-3 text-center mypd-student-dashboard-grade">';
            $userboard .= '<h4 class="h6">' . get_string('grade') . '</h6>';
            $userboard .= '<a href="' . $moodleurl . '">';
            $userboard .= '<div class="js-progressbar-circle mypd-progress-circle mypd-progressbar-link" value="';
            $userboard .= s($coursegrade) . '"></div>';
            $userboard .= '</a>';
            $userboard .= '</div>';
        }

        $userboard .= '</div><!- close .mypd-user-dashboard ->';
        $userboard .= '<br>';
