...
use moodle_url;
use stdClass;
use theme_mypd\renderables\course_action_section_move;
use theme_mypd\renderables\course_action_section_visibility;
use theme_mypd\renderables\course_action_section_delete;
use theme_mypd\renderables\course_action_section_highlight;
use theme_mypd\renderables\course_section_navigation;
...
trait format_section_trait {
    ...
    protected function section_header($section, $course, $onsectionpage, $sectionreturn=null) {
        ...
        // ajax.
        $output = $PAGE->get_renderer('theme_mypd', 'core', RENDERER_TARGET_GENERAL);
        ...
        if ($sectiontitle == get_string('general') && $section->section == 0) {
            $classes = '';
            $sectiontitle = get_string('introduction', 'theme_mypd');
        }
        ...
        if ($sectiontitle == $testemptytitle && has_capability('moodle/course:update', $context)) {
            ...
            $o .= "<h2 class='sectionname'><a href='$url' title='".s(get_string('editcoursetopic', 'theme_mypd'))."'>";
            $o .= get_string('defaulttopictitle', 'theme_mypd')."</a></h2>";
        } else {
        ...
        if ($caneditcourse && $section->section != 0) {
            $o .= "<a class='mypd-drop section-drop' data-title='".
        ...
        if (has_capability('moodle/course:update', $context)) {
            if (!empty($sectiontoolsarray)) {
                ...
                $o .= html_writer::tag('div', $sectiontools, array(
                    'class' => 'js-only mypd-section-editing actions',
                    'role' => 'region',
                    'aria-label' => get_string('topicactions', 'theme_mypd')
                ));
            }
        }
        // Draft message.
        $drafticon = '<img aria-hidden="true" role="presentation" class="svg-icon" src="'.$output->image_url('/i/show').'" />';
        $o .= '<div class="mypd-draft-tag mypd-draft-section">'.$drafticon.' '.get_string('draft', 'theme_mypd').'</div>';

        // Current section message.
        $currenticon = '<img aria-hidden="true" role="presentation" class="svg-icon" src="'.$output->image_url('/i/marked').'" />';
        $o .= '<span class="mypd-current-tag">'.$currenticon.' '.get_string('current', 'theme_mypd').'</span>';
        ...
        if ($formattedinfo !== '') {
            $o .= '<div class="mypd-conditional-tag">'.$conditionalicon.' '.$formattedinfo.'</div>';
        }
        ...
        if (empty($summarytext) && $canupdatecourse) {
            $summarytext = "<p tabindex='0'>".get_string('defaultsummary', 'theme_mypd')."</p>";
            if ($section->section == 0) {
                $editorname = format_string(fullname($USER));
                $summarytext = "<p tabindex='0'>".get_string('defaultintrosummary', 'theme_mypd', $editorname)."</p>";
            }
        } else {
        ...
        if ($canupdatecourse) {
            ...
            $o .= '<a href="'.$url.'" class="edit-summary">'.$icon.get_string('editcoursetopic', 'theme_mypd'). '</a>';
        }
        $o .= "</div>";
    ...
    public function render_course_section_navigation(course_section_navigation $navigation) {
        return $this->render_from_template('theme_mypd/course_section_navigation', $navigation);
    }
    ...
    private function change_num_sections($course) {
        ...
        $url = new moodle_url('/theme/mypd/index.php', array(
            'sesskey'  => sesskey(),
            'action' => 'addsection',
        ...
        $required = '';
        $defaulttitle = get_string('title', 'theme_mypd');
        ...
        $heading = get_string('addanewsection', 'theme_mypd');
        $output = "<section id='mypd-add-new-section' class='clearfix' tabindex='-1'>
        <h3>$heading</h3>";
        ...
        $output .= "<label for='newsection' class='sr-only'>".get_string('title', 'theme_mypd')."</label>";
        if ($course->format === 'topics') {
            $output .= '<input class="h3" id="newsection" type="text" maxlength="250" name="newsection" '.$required;
            $output .= ' placeholder="'.s(get_string('title', 'theme_mypd')).'">';
        } else {
            $output .= '<h3>'.$defaulttitle.': '.$datesection.'</h3>';
        }
        $output .= '</div>';
        $output .= '<div class="form-group">';
        $output .= '<label for="summary">'.get_string('contents', 'theme_mypd').'</label>';
        ...
        $output .= html_writer::empty_tag('input', array(
            ...
            'name' => 'addtopic',
            'value' => get_string('createsection', 'theme_mypd'),
        ));
    ...
    public function course_section_add_cm_control($course, $section, $sectionreturn = null, $displayoptions = array()) {
        ...
        $mcdataattributes = 'data-section="'.$section.'" data-toggle="modal" data-target="#mypd-modchooser-modal"';
        $modchooser = '
        <div class="col-sm-6 mypd-modchooser">
            <a href="#" class="'.$mcclass.'" '.$mcdataattributes.'>'.$icon.get_string('addresourceoractivity', 'theme_mypd').'</a>
        </div>';

        // Add zone for quick uploading of files.
        $upload = '<div class="col-sm-6">
            <form class="mypd-dropzone js-only">
                <label tabindex="0" for="mypd-drop-file-'.$section.'" class="mypd-dropzone-label">';
        $upload .= get_string('dropzonelabel', 'theme_mypd').'</label>
                <input type="file" multiple name="mypd-drop-file-'.$section.'" id="mypd-drop-file-'.$section;
        $upload .= '" class="js-mypd-drop-file sr-only"/>
            </form>
            </div>';
    ...
    /**
     * Always output the html for multiple sections, single section mode is not supported in myPD.
     *
     * @param stdClass $course The course entry from DB
    ...
    public function render_course_action_section_move(course_action_section_move $action) {
        ...
        return $this->render_from_template('theme_mypd/course_action_section', $data);
    }
