...
use navigation_node;
use user_picture;
use theme_mypd\local;
use theme_mypd\activity;
use theme_mypd\services\course;
use theme_mypd\renderables\settings_link;
use theme_mypd\renderables\bb_dashboard_link;
use theme_mypd\renderables\course_card;
use theme_mypd\renderables\course_toc;
use theme_mypd\renderables\featured_courses;

// We have to force include this class as it's on login and the auto loader may not have been updated via a cache dump.
require_once($CFG->dirroot.'/theme/mypd/classes/renderables/login_alternative_methods.php');
use theme_mypd\renderables\login_alternative_methods;
...
class core_renderer extends \theme_boost\output\core_renderer {
        ...
        if ($helpidentifier) {
            ...
            if ($helpidentifier === 'modulename') {
                ...
                if (!empty($mod) && isset($mod[$component]) && is_object($mod[$component]) && $mod[$component]->help) {
                    ...
                    $collapsablehelp = $this->render_from_template('theme_mypd/heading_help_collapse', $data);
                    $classnames .= ' d-inline';
                }
    ...
    public function course_toc() {
        ...
        return $this->render_from_template('theme_mypd/course_toc', $coursetoc);
    }
    ...
    public function get_course_image() {
        ...
        return \theme_mypd\local::course_coverimage_url($COURSE->id);
    }
    ...
    public function column_header_icon_link($langstring, $iconname, $url) {
        global $OUTPUT;
        $text = get_string($langstring, 'theme_mypd');
        $iconurl = $OUTPUT->image_url($iconname, 'theme');
        $icon = '<img class="svg-icon" role="presentation" src="' .$iconurl. '">';
        $link = '<a class="mypd-personal-menu-more" href="' .$url. '"><small>' .$text. '</small>' .$icon. '</a>';
        return $link;
    }
    ...
    public function mobile_menu_link($langstring, $iconname, $url) {
        global $OUTPUT;
        $alt = get_string($langstring, 'theme_mypd');
        $iconurl = $OUTPUT->image_url($iconname, 'theme');
    ...
    protected function render_badge_count() {
        ...
        if (!empty($CFG->messaging) && !empty(get_config('theme_mypd', 'messagestoggle'))) {
    ...
    protected function render_messages() {
        ...
        $messagesheading = get_string('messages', 'theme_mypd');
        $o = '<h2>'.$messagesheading.'</h2>';
        $o .= '<div id="mypd-personal-menu-messages"></div>';
    ...
    protected function render_forumposts() {
        ...
        $forumpostsheading = get_string('forumposts', 'theme_mypd');
        $o = '<h2>'.$forumpostsheading.'</h2>
        <div id="mypd-personal-menu-forumposts"></div>';
     ...
     * @return string
     */
    public function mypd_media_object($url, $image, $title, $meta, $content, $extraclasses = '', $attributes = '') {
        ...
        if (is_array($meta)) {
            $metastr = '<span class="mypd-media-meta">';
            foreach ($meta as $metaitem) {
                $metastr .= $metaitem.'<br>';
            }
            $metastr .= '</span>';
        } else if ($meta) {
            $metastr = '<span class="mypd-media-meta">' .$meta.'</span>';
        }
        ...
        return $this->render_from_template('theme_mypd/media_object', $data);
    }
    ...
    /**
     * Output moodle blocks and myPD wrapper with edit button.
     * @return string
     */
    public function mypd_blocks() {
        ...
        if ($oncoursepage && has_capability('moodle/course:update', $coursecontext)) {
            ...
            if ($PAGE->user_is_editing()) {
                ...
                $url->param('edit', 'on');
                $editstring = get_string('editcoursecontent', 'theme_mypd');
            }
    ...
    protected function render_callstoaction() {
        ...
        $mobilemenu = '<div id="mypd-pm-mobilemenu">';
        $mobilemenu .= $this->mobile_menu_link('courses', 'courses', '#mypd-pm-courses');
        $deadlines = $this->render_deadlines();
        if (!empty($deadlines)) {
            $columns[] = $deadlines;
            $mobilemenu .= $this->mobile_menu_link('deadlines', 'calendar', '#mypd-personal-menu-deadlines');
        }

        $graded = $this->render_graded();
        $grading = $this->render_grading();
        if (empty($grading)) {
            $gradebookmenulink = $this->mobile_menu_link('recentfeedback', 'grading', '#mypd-personal-menu-graded');
        } else {
            $gradebookmenulink = $this->mobile_menu_link('grading', 'grading', '#mypd-personal-menu-grading');
        }
        if (!empty($grading)) {
            $columns[] = $grading;
        ...
        if (!empty($messages)) {
            $columns[] = $messages;
            $mobilemenu .= $this->mobile_menu_link('messages', 'messages', '#mypd-personal-menu-messages');
        }

        $forumposts = $this->render_forumposts();
        if (!empty($forumposts)) {
            $columns[] = $forumposts;
            $mobilemenu .= $this->mobile_menu_link('forumposts', 'forumposts', '#mypd-personal-menu-forumposts');
        }
    ...
    protected function render_grading() {
        ...
        $gradingheading = get_string('grading', 'theme_mypd');
        $o = "<h2>$gradingheading</h2>";
        $o .= '<div id="mypd-personal-menu-grading"></div>';
    ...
    protected function render_graded() {
        ...
        $recentfeedback = get_string('recentfeedback', 'theme_mypd');
        $o = "<h2>$recentfeedback</h2>";
        $o .= '<div id="mypd-personal-menu-graded"></div>';
    ...
    protected function render_deadlines() {
        ...
        $deadlinesheading = get_string('deadlines', 'theme_mypd');
        $o = "<h2>$deadlinesheading</h2>";
        $o .= '<div id="mypd-personal-menu-deadlines"></div>';
    ...
    public function login_button() {
        ...
        $loginatts = [
            'aria-haspopup' => 'true',
            'class' => 'btn btn-primary mypd-login-button js-mypd-pm-trigger',
        ];
        if (!empty($CFG->alternateloginurl) or !empty($CFG->theme_mypd_disablequicklogin)) {
            $loginurl = $CFG->wwwroot.'/login/index.php';
            $loginatts = [
                'class' => 'btn btn-primary mypd-login-button',
            ];
        }
    ...
    public function render_course_card(course_card $card) {
        return $this->render_from_template('theme_mypd/course_cards', $card);
    }
    ...
    public function render_login_alternative_methods(login_alternative_methods $methods) {
        ...
        return $this->render_from_template('theme_mypd/login_alternative_methods', $methods);
    }
    ...
    public function personal_menu() {
        ...
        if (!isloggedin() || isguestuser()) {
            ...
            if ($PAGE->pagetype !== 'login-index') {
                return $this->render_from_template('theme_mypd/login', $data);
            } else {
                return '';
            }
        }
        ...
        $fullnamelink = '<a href="' .s($CFG->wwwroot). '/user/profile.php"
                    title="' .s(get_string('viewyourprofile', 'theme_mypd')). '"
        ...
        if (\core\session\manager::is_loggedinas()) {
            ...
            $realfullnamelink = '<br>' .get_string('via', 'theme_mypd'). ' ' .format_string(fullname($realuser, true));
        }
        ...
        return $this->render_from_template('theme_mypd/personal_menu', $data);
    }
    ...
    public function personal_menu_trigger() {
        ...
        if (!isloggedin() || isguestuser()) {
            ...
            $menu = '<span class="hidden-xs-down">' .get_string('menu', 'theme_mypd'). '</span>';
            $badge = $this->render_badge_count();
            $linkcontent = $picture.$menu.$badge;
            $attributes = array(
                'aria-haspopup' => 'true',
                'class' => 'js-mypd-pm-trigger mypd-my-courses-menu',
                'id' => 'mypd-pm-trigger',
                'aria-controls' => 'mypd-pm',
            );
    ...
    public function cover_image_selector() {
        ...
        if (has_capability('moodle/course:changesummary', $PAGE->context)) {
            ...
            return $this->render_from_template('theme_mypd/cover_image_selector', $vars);
        }
        return null;
    }
    ...
    public function cover_carousel() {
        ...
        $data['slides'] = $slides;
        return $this->render_from_template('theme_mypd/carousel', $data);
    }
    ...
    public function page_heading($tag = 'h1') {
        ...
        if ($this->page->pagelayout == 'frontpage') {
            $heading .= '<p class="mypd-site-description">' . format_string($this->page->theme->settings->subtitle) . '</p>';
        }
        if ($this->page->user_is_editing() && $this->page->pagelayout == 'frontpage') {
            $url = new moodle_url('/admin/settings.php', ['section' => 'themesettingmypd']);
            $link = html_writer::link($url,
                            get_string('changefullname', 'theme_mypd'),
    ...
    public function site_frontpage_news() {
        ...
        foreach ($discussions as $discussion) {
            ...
            $imgarr = \theme_mypd\local::extract_first_image($message);
            if ($imgarr) {
            ...
            $readmorebtn = "<a class='btn btn-secondary toggle' href='".
                ...
                get_string('readmore', 'theme_mypd')."</a>";
            ...
            if (!$imagestyle) {
                ...
                $newsimage = '<div class="news-article-image toggle"'.$imagestyle.' title="'.
                    get_string('readmore', 'theme_mypd').'"></div>';
            }
...
<div class="news-article clearfix">
    ...
    <div class="news-article-message" tabindex="-1">
        {$message}
        <div><hr><a class="mypd-action-icon mypd-icon-close toggle" href="#">
...
HTML;
        ...
        $actionlinks = html_writer::link(
            ...
            get_string('morenews', 'theme_mypd'),
    ...
    /**
     * add in additional classes that are used for myPD
    ...
    public function body_css_classes(array $additionalclasses = array()) {
        ...
        if (!empty($PAGE->theme->settings->resourcedisplay)) {
            $classes[] = 'mypd-resource-'.$PAGE->theme->settings->resourcedisplay;
        } else {
            $classes[] = 'mypd-resource-card';
        }

        // Add theme-mypd class so modules can customise css for mypd.
        $classes[] = 'theme-mypd';
    ...
    public function confirm($message, $continue, $cancel) {
        ...
        $output = $this->box_start('generalbox mypd-continue-cancel', 'notice');
    ...
    public function recent_forum_activity(Array $activities) {
        ...
        foreach ($activities as $activity) {
            ...
            $output .= $this->mypd_media_object($url, $picture, $fullname, $meta, $formattedsubject);
        }
        return $output;
    }
    ...
    public function deadlines(stdClass $eventsobj) {
        ...
        if (empty($events)) {
            return '<p class="small"'.$datafromcache.'>' . get_string('nodeadlines', 'theme_mypd') . '</p>';
        }
        ...
        foreach ($events as $event) {
            if (!empty($event->modulename)) {
                ...
                if (!has_capability('mod/assign:grade', $cm->context)) {
                    /** @var \theme_mypd_core_course_renderer $courserenderer */
                    $courserenderer = $PAGE->get_renderer('core', 'course', RENDERER_TARGET_GENERAL);
                    $activitymeta = activity::module_meta($cm);
                    $meta .= '<div class="mypd-completion-meta">' .
                        $courserenderer->submission_cta($cm, $activitymeta) .
                        '</div>';
                }
                $o .= $this->mypd_media_object($cm->url, $modimage, $eventtitle, $meta, '', '', $datafromcache);
            }
        }
        if (empty($o)) {
            return '<p>' . get_string('nodeadlines', 'theme_mypd') . '</p>';
        }
        return $o;
    }
    ...
    public function feature_spot_cards() {
        ...
        foreach ($fsnames as $feature) {
            ...
            if (!empty($PAGE->theme->settings->$title) && !empty($PAGE->theme->settings->$text)) {
                ...
                if (!empty($PAGE->theme->settings->$image)) {
                    ...
                    $img = '<!--Card image-->
                    <img class="mypd-feature-image" src="' .$url. '" alt="" role="presentation">';
                }
        ...
        if ($fscount > 0) {
            ...
            if (!empty($PAGE->theme->settings->fs_heading)) {
                $fstitle = '<h2 class="mypd-feature-spots-heading">' .s($PAGE->theme->settings->fs_heading). '</h2>';
            }
            ...
            foreach ($features as $feature) {
                $cards .= '<div class="' .$colclass. '" id="mypd-feature-' .$i. '">' .$feature. '</div>';
                $i++;
            }

            $fsedit = '';
            if ($this->page->user_is_editing()) {
                $url = new moodle_url('/admin/settings.php?section=themesettingmypd#thememypdfeaturespots');
                $link = html_writer::link($url, get_string('featurespotsedit', 'theme_mypd'), ['class' => 'btn btn-primary']);
                $fsedit = '<p class="text-center">'.$link.'</p>';
            }

            // Build feature spots.
            $featurespots = '<div id="mypd-feature-spots">';
    ...
    protected function feature_spot_card($title, $image, $text) {
        $card = '<div class="mypd-feature">
            <!--Card content-->
            <div class="mypd-feature-block">
                ' .$image. '
                <!--Title-->
                <h3 class="mypd-feature-title h5">' .s($title). '</h3>
                <!--Content-->
                <p class="mypd-feature-text">' .format_text($text). '</p>
            </div>
            <!--/.Card content-->
        </div>';
        return $card;
    ...
    public function render_featured_courses(featured_courses $fc) {
        ...
        return $this->render_from_template('theme_mypd/featured_courses', $fc);
    }

    /**
     * Return mypd modchooser modal.
     * @return string
     */
    protected function course_modchooser() {
        ...
        return $this->render_from_template('theme_mypd/course_modchooser_modal', $data);
    }
    ...
    /**
     * Return myPD's logo url for login.mustache
     *
     * @param int $maxwidth not used in myPD.
     * @param int $maxheight not used in myPD.
    ...
    public function get_logo_url($maxwidth = null, $maxheight = 200) {
        ...
        $url = moodle_url::make_file_url("$CFG->httpswwwroot/pluginfile.php", "/$syscontextid/theme_mypd/logo/$itemid".$filepath);
        return $url;
    }
    ...
    protected function render_intelliboard() {
        ...
        $o .= '<div id="mypd-personal-menu-intelliboard">'
                .$links.
                '</div>';
    ...
    protected function render_intellicart() {
        ...
        $o .= '<div id="mypd-personal-menu-intellicart">'
            .$link.
            '</div>';
