...
/**
 * myPD page requirements manager.
...
class mypd_page_requirements_manager extends \page_requirements_manager {
    /**
     ...
     * the mypd_page_requirements_manager.
     */
    public function copy_page_requirements() {
        ...
        foreach ($props as $prop) {
            ...
            $mypdmanprop = new \ReflectionProperty($this, $pname);
            // If the property is private or protected  set accessible, after the copy reset to not accessible.
            $isprotected = $mypdmanprop->isPrivate() || $mypdmanprop->isProtected();
            if ($isprotected) {
                $mypdmanprop->setAccessible(true);
            }
            $mypdmanprop->setValue($this, $pval);
            $mypdmanprop->setAccessible(!$isprotected);
        }
