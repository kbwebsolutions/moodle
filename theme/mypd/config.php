...
use theme_mypd\mypd_page_requirements_manager;

global $SESSION, $COURSE, $USER, $PAGE;

$theme = local::resolve_theme();
$themeismypd = $theme === 'mypd';
$notajaxscript = !defined('AJAX_SCRIPT') || AJAX_SCRIPT == false;
// The code inside this conditional block is to be executed prior to page rendering when the theme is set to mypd and
...
if ($themeismypd && $notajaxscript) {
    ...
    if (empty($SESSION->theme_mypd_last_course) || $SESSION->theme_mypd_last_course != $COURSE->id) {
        $USER->editing = 0;
        $SESSION->theme_mypd_last_course = $COURSE->id;
    }

    if (isset($SESSION->wantsurl)) {
        // We are taking a backup of this because it can get unset later by core.
        $SESSION->mypdwantsurl = $SESSION->wantsurl;
    }
}
...
$THEME->prescsscallback = 'theme_mypd_get_pre_scss';
$THEME->scss = function($theme) {
    return theme_mypd_get_main_scss_content($theme);
};
$THEME->csspostprocess = 'theme_mypd_process_css';
...
if ($themeismypd && $notajaxscript) {
    if (empty($CFG->mypdpageinit) && !empty($PAGE)) {
        $CFG->mypdpageinit = true;
        $PAGE->initialise_theme_and_output();

        // Modify $PAGE to use mypd requirements manager.
        $mypdpman = new mypd_page_requirements_manager();
        $mypdpman->copy_page_requirements();
    }
