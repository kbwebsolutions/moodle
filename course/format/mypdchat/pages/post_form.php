...
class post_form extends moodleform {
    ...
    protected function definition() {
        ...
        $cache = cache::make('format_mypdchat', 'postformparams');
        ...
        $errorcache = cache::make('format_mypdchat', 'postformerrors');
        ...
        // ... posttext.
        $buttongroup = array();
        $buttongroup[] = $mform->createElement('submit', 'submitbutton', get_string($action, 'format_mypdchat'));
        ...
        $canposthtml = has_capability('format/mypdchat:posthtml', $context);
        ...
        if ($showeditor) {
            ...
            $mform->addElement('editor', 'posttext', get_string('poststatusordnote', 'format_mypdchat'), $params);
            ...
            $mform->addElement('textarea', 'posttext', get_string('poststatusordnote', 'format_mypdchat'), $params);
        ...
        // ... options group.
        $poststatusmenu = array(0 => get_string('poststatus', 'format_mypdchat'));
        if (has_capability('format/mypdchat:makesticky', $context)) {
            $poststatusmenu[1] = get_string('makesticky', 'format_mypdchat');
        }
        if (has_capability('format/mypdchat:postprivate', $context)) {
            $poststatusmenu[2] = get_string('privatepost', 'format_mypdchat');
        }
        if ($PAGE->user_allowed_editing()) {
            $poststatusmenu[4] = get_string('makealert', 'format_mypdchat');
        }
        ...
        if ($canposthtml) {
            ...
            $postoptions[] = $mform->createElement('submit', $key, get_string($key, 'format_mypdchat'));
        }
        ...
        if ($PAGE->user_allowed_editing()) {
            ...
            if (!isset($USER->editing) or ( !$USER->editing)) {
                ...
                $addstr = get_string('addactivityresource', 'format_mypdchat');
            ...
            $canpostfile = (has_capability('format/mypdchat:postfile', $context) && (!empty($course->enablestudentupload)));

            if ($canpostfile) {

                $uploadfileicon = $OUTPUT->pix_icon('icon', get_string('uploadafile', 'format_mypdchat'), 'resource');
                $linktext = $uploadfileicon . get_string('uploadafile', 'format_mypdchat');
            ...
            $canposturl = (has_capability('format/mypdchat:posturl', $context) && (!empty($course->enablestudentupload)));

            if ($canposturl) {
                $addlinkicon = $OUTPUT->pix_icon('icon', get_string('addalink', 'format_mypdchat'), 'url');
                $at = html_writer::link('#', $addlinkicon . get_string('addalink', 'format_mypdchat'), array('id' => 'addalink'));
    ...
    public function validation($data, $files) {
        ...
        $cache = cache::make('format_mypdchat', 'postformerrors');
        ...
        if (isset($data['submitbutton'])) {
            ...
            if ($empty) {
                $errors['posttext'] = get_string('attachmentorpostrequired', 'format_mypdchat');
    ...
    public function set_data($post) {
        ...
        if ($cmids = $DB->get_records('format_mypdchat_attaches', array('postid' => $post->id), '', 'coursemoduleid')) {
