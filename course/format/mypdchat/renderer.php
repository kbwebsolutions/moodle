...
require_once($CFG->dirroot . '/course/format/mypdchat/pages/comment_form.php');

/**
 * Basic renderer for mypdchat format.
 *
 * @copyright 2014 Andreas Wagner, Synergy Learning
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_mypdchat_renderer extends format_topics_renderer {
    ...
    private function get_timeline_author($userid, $authors) {
        ...
        if (isset($authors[$userid])) {
            ...
            $postauthor = (object) array('firstname' => '', 'lastname' => get_string('unknownuser', 'format_mypdchat'),
                        'firstnamephonetic' => '', 'lastnamephonetic' => '', 'middlename' => '', 'alternatename' => '');

            $o = $this->output->pix_icon('u/user35', get_string('unknownuser', 'format_mypdchat'));
        }
    ...
    protected function render_timeline_comment_ago($time) {
        ...
        if ($seconds < 60) {
            return get_string('timeagosec', 'format_mypdchat', $seconds);
        }

        $minutes = floor($seconds / 60);

        if ($minutes < 60) {
            return get_string('timeagomin', 'format_mypdchat', $minutes);
        }
        ...
        if ($hours < 24) {
            return get_string('timeagohours', 'format_mypdchat', array('minutes' => $minutes, 'hours' => $hours));
        }
        ...
        if ($days < 30) {
            return get_string('timeagodays', 'format_mypdchat', array('days' => $days, 'hours' => $hours));
        }
        ...
        if ($days < 300) {
            return $interval->format(get_string('timeagomonthdays', 'format_mypdchat'));
        }

        return $interval->format(get_string('timeagoyearsdays', 'format_mypdchat'));
    }
    ...
    protected function render_timeline_comment($post, $comment, $authors,
        ...
        $candeletecomment = ($candeletecomment and has_capability('format/mypdchat:deleteowncomment', $coursecontext));
        $candeletecomment = ($candeletecomment or has_capability('format/mypdchat:deleteanycomment', $coursecontext));
        ...
        if ($candeletecomment) {
            ...
            $url = new moodle_url('/course/format/mypdchat/action.php', $urlparams);
        ...
        if ($morerepliescount > 0) {
            ...
            $url = new moodle_url('/course/format/mypdchat/action.php');
            $strmore = get_string('showallreplies', 'format_mypdchat', $morerepliescount);
    ...
    protected function render_timeline_grades($authors, $gradedata) {
        ...
        $c .= html_writer::tag('div', get_string('gradednote', 'format_mypdchat', $a));
    ...
    protected function render_timeline_comments_form($actionlink,
        ...
        $capcomment = (has_capability('format/mypdchat:writecomment', $coursecontext));
        ...
        if ($capcomment and $canreply) {
            $commenticon = $this->output->pix_icon('comment', get_string('comment', 'format_mypdchat'), 'format_mypdchat');
            ...
            $urltext = (empty($replycommentid)) ? get_string('writecomment', 'format_mypdchat') : get_string('replycomment', 'format_mypdchat');
            $actionlink .= html_writer::tag('span', $commenticon, array('class' => 'tl-commenticon'));
        ...
        if ($capcomment and $canreply) {
            ...
            $url = new moodle_url('/course/format/mypdchat/action.php', $formparams);
    ...
    protected function render_timeline_post($course, $post, $completion,
        ...
        if (empty($post->togroupid)) {
            ...
            $to = (isset($groups[$post->togroupid])) ?
                    ...
                    get_string('nonexistinggroup', 'format_mypdchat');
        }

        $date = userdate($post->timecreated);
        $authorspan = html_writer::tag('div', fullname($postauthor), array('class' => 'tl-authorname'));
        $headline = get_string('postedonto', 'format_mypdchat', array('author' => $authorspan, 'date' => $date, 'to' => $to));

        // ... add delete icon, if delete is possible.
        $candeletepost = (($post->fromuserid == $USER->id) and ( has_capability('format/mypdchat:deleteownpost', $coursecontext)));
        $candeletepost = ($candeletepost or has_capability('format/mypdchat:deleteanypost', $coursecontext));
        ...
        $caneditpost = (($post->fromuserid == $USER->id) and ( has_capability('format/mypdchat:updateownpost', $coursecontext)));
        $caneditpost = ($caneditpost or has_capability('format/mypdchat:updateanypost', $coursecontext));
        ...
        if (!empty($post->posttext)) {
            // ...user with cap format/mypdchat:posthtml should be able to html.
        ...
        if (has_capability('format/mypdchat:lockcomment', $coursecontext)) {
            ...
            if (!empty($post->locked)) {
                ...
                $pixicon = $this->output->pix_icon('lockedpost', get_string('unlockpost', 'format_mypdchat'), 'format_mypdchat');
            } else {

                $pixicon = $this->output->pix_icon('unlockedpost', get_string('lockpost', 'format_mypdchat'), 'format_mypdchat');
            }
        ...
        if (has_capability('format/mypdchat:makesticky', $coursecontext)) {
            ...
            if (!empty($post->sticky)) {
                ...
                $pixicon = $this->output->pix_icon('stickypost', get_string('makeunsticky', 'format_mypdchat'), 'format_mypdchat');
            } else {

                $pixicon = $this->output->pix_icon('unstickypost', get_string('makesticky', 'format_mypdchat'), 'format_mypdchat');
            }
            ...
            if (!empty($post->sticky)) {
                ...
                $pixicon = $this->output->pix_icon('stickypost', get_string('sticky', 'format_mypdchat'), 'format_mypdchat');
        ...
        if ($post->timecreated != $post->timemodified) {
            ...
            $c = html_writer::tag('div', get_string('edited', 'format_mypdchat'), array('class' => 'tl-edited'));
        ...
        $countoutput = '';
        if (!empty($course->enablelikes) and has_capability('format/mypdchat:viewlikes', $coursecontext)) {

            $likepix = $this->output->pix_icon('like', get_string('like', 'format_mypdchat'), 'format_mypdchat');
            $countlikessstr = get_string('countlikes', 'format_mypdchat', $post->countlikes);
            $countoutput .= html_writer::tag('span', $likepix);
            $countoutput .= html_writer::tag('span', $countlikessstr, array('id' => 'tlcountlikes_' . $post->id));
        }

        $countcommentsstr = get_string('countcomments', 'format_mypdchat', $post->countcomments);
        $countoutput .= html_writer::tag('span', $commentpic);
        $countoutput .= html_writer::tag('div', $countcommentsstr, array('id' => 'tlcountcomments_' . $post->id, 'class' => 'tl-countcomments'));

        $actionarea = html_writer::tag('div', $countoutput, array('class' => 'tl-counts'));

        $stralldiscussions = get_string('showalldicussions', 'format_mypdchat');
        ...
        if (!empty($course->enablelikes) and has_capability('format/mypdchat:like', $coursecontext)) {
            ...
            if (empty($post->userlike)) {
                $pixicon = $this->output->pix_icon('likeOn', get_string('like', 'format_mypdchat'), 'format_mypdchat');
            } else {
                $pixicon = $this->output->pix_icon('likeOff', get_string('like', 'format_mypdchat'), 'format_mypdchat');
            }
            $url = new moodle_url('/course/format/mypdchat/action.php', $urlparams);
            $urlparams = array('class' => $class, 'id' => "userlike_{$post->id}");
            $actionlink .= html_writer::tag('span', $pixicon, array('class' => 'tl-like'));
            $actionlink .= html_writer::link($url, get_string($class, 'format_mypdchat'), $urlparams);
        }
        ...
        if ($morecommentscount > 0) {
            ...
            $url = new moodle_url('/course/format/mypdchat/action.php');
            $strmore = get_string('showallcomments', 'format_mypdchat', $morecommentscount);
    ...
    protected function render_timeline_filterform($course, $filteroptions) {
        ...
        if (!empty($filteroptions->showalert)) {
            ...
            $content = html_writer::tag('h4', get_string('showalert', 'format_mypdchat'));
            $url = new moodle_url('/course/view.php', array('id' => $course->id));

            $content .= $this->output->single_button($url, get_string('showallposts', 'format_mypdchat'));
        ...
        if (!empty($filteroptions->postid)) {
            ...
            $content = html_writer::tag('h4', get_string('updatepostfiltered', 'format_mypdchat'));
            $url = new moodle_url('/course/format/mypdchat/action.php', array('courseid' => $course->id, 'action' => 'resetfilter'));
        ...
        if ((groups_get_course_groupmode($course) == SEPARATEGROUPS) && (!has_capability('moodle/course:managegroups', $coursecontext))) {
            ...
            $alllabel = get_string('allmygroups', 'format_mypdchat');
        } else {
        ...
        $nothing = array('' => get_string('allmodultypes', 'format_mypdchat'));
        $f .= html_writer::select($modulenames, 'tl_filtermodules', $filteroptions->filtermodules, $nothing);

        // ... order by date.
        $options = array();
        $options['timecreated asc'] = get_string('timecreateasc', 'format_mypdchat');

        $nothing = array('timecreated desc' => get_string('timecreatedesc', 'format_mypdchat'));
        $f .= html_writer::select($options, 'tl_orderby', $filteroptions->orderby, $nothing);

        $inputparams = array(
            'type' => 'submit', 'name' => 'filter',
            'value' => get_string('filtertimeline', 'format_mypdchat')
        );
    ...
    protected function print_postform_section($course, $sectioninfo, $postform,
        ...
        if (isset($sectioninfo[FORMAT_MYPDCHAT_POSTFORMSECTION])) {

            $thissection = $sectioninfo[FORMAT_MYPDCHAT_POSTFORMSECTION];
            ...
            $url = new moodle_url('/course/format/mypdchat/pages/editnotification.php', array('courseid' => $course->id));

            $linktext = $this->output->pix_icon('i/settings', get_string('editnotification', 'format_mypdchat')) . " " .
                    get_string('editnotification', 'format_mypdchat');
            ...
            if ($USER->editing) {
                ...
                $o .= get_string('attachactivities', 'format_mypdchat');

                if (!empty($thissection->sequence)) {

                    $o .= html_writer::empty_tag('br') . get_string('attachedactivities', 'format_mypdchat');
                }
                $o .= $this->courserenderer->course_section_cm_list($course, $thissection, 0);
                $o .= $this->courserenderer->course_section_add_cm_control($course, FORMAT_MYPDCHAT_POSTFORMSECTION, 0);
            ...
            if ($USER->editing or $postid > 0) {
                ...
                if ($USER->editing) {
                    ...
                    $l = $this->output->pix_icon('t/add', get_string('addrecentactivity', 'format_mypdchat'));
                    $l .= " ".get_string('addrecentactivity', 'format_mypdchat');
                ...
                if (($USER->editing) or (count($attachedrecentactivities) > 0)) {
                    ...
                    $content = get_string('attachedrecentactivities', 'format_mypdchat').$content;
    ...
    protected function render_postform_recent_activities_form($course, $postid) {
        ...
        require_once($CFG->dirroot . '/course/format/mypdchat/pages/addactivity_form.php');
    ...
    protected function print_timeline_section($course, $postsdata, $completion) {
        ...
        $countstr = get_string('counttotalpost', 'format_mypdchat', $countdata);
    ...
    public function print_page($course, $completioninfo) {
        ...
        $posts = \format_mypdchat\local\posts::instance($course->id);
        ...
        $this->page->requires->strings_for_js(
                array('counttotalpost', 'like', 'likenomore', 'countlikes', 'countcomments', 'textrequired', 'confirmdeletecomment'), 'format_mypdchat');

        $this->page->requires->yui_module(
                'moodle-format_mypdchat-postform', 'M.format_mypdchat.postforminit', array($args), null, true);
    }
