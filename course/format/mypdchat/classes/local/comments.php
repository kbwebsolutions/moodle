...
namespace format_mypdchat\local;
...
class comments {
    ...
    public static function add_comments_to_posts(&$postsdata,
        ...
        if (!$replies = $DB->get_records_list('format_mypdchat_comments', 'replycommentid', array_keys($parentcommentids), 'timecreated DESC')) {
            return true;
        }
    ...
    public function get_replies_data($comment) {
        ...
        if (!$replies = $DB->get_records('format_mypdchat_comments', array('replycommentid' => $comment->id), 'timecreated DESC')) {
            return $repliesdata;
        }
    ...
    public function get_comments_data($postid, $limitcomments = 0, $limitreplies = 0) {
        ...
        if (!$postsdata->posts = $DB->get_records('format_mypdchat_posts', array('id' => $postid))) {
    ...
    private function refresh_replies_count($commentid) {
        ...
        if ($comment = $DB->get_record('format_mypdchat_comments', array('id' => $commentid))) {

            $comment->countreplies = $DB->count_records('format_mypdchat_comments', array('replycommentid' => $commentid));
            $comment->timemodified = time();

            $DB->update_record('format_mypdchat_comments', $comment);
            return $comment;
        }
        return false;
    }
    ...
    public function save_comment_from_submit($comment, $course) {
        ...
        if (!$post = $DB->get_record('format_mypdchat_posts', array('id' => $comment->postid))) {
        ...
        if (!has_capability('format/mypdchat:writecomment', $coursecontext)) {
            print_error('missingcapwritecomment', 'format_mypdchat');
        }
        ...
        if ($post->locked != 0) {
            print_error('postislocked', 'format_mypdchat');
        }

        // ...insert comment.
        if (!$comment->id = $DB->insert_record('format_mypdchat_comments', $comment)) {
            print_error('commentsaveerror', 'format_mypdchat');
        }
    ...
    public function delete_comment($cid) {
        ...
        if (!$comment = $DB->get_record('format_mypdchat_comments', array('id' => $cid))) {
            print_error('commentidinvalid', 'format_mypdchat');
        }

        // ...check capability.
        $coursecontext = \context_course::instance($comment->courseid);

        $candeletecomment = (($comment->fromuserid == $USER->id) and ( has_capability('format/mypdchat:deleteowncomment', $coursecontext)));
        $candeletecomment = ($candeletecomment or has_capability('format/mypdchat:deleteanycomment', $coursecontext));

        if (!$candeletecomment) {
            print_error('missingcapdeletecomment', 'format_mypdchat');
        }

        // ... delete comment.
        $DB->delete_records('format_mypdchat_comments', array('id' => $cid));

        // ... delete all the enqueued notifications.
        $DB->delete_records_select('format_mypdchat_nfqueue', "module = 'comment' and details = ?", array($cid));
